const items =
{
"1": {
  "id": 1,
  "name": "item_blink",
  "displayName": "Blink Dagger",
  "shortName": "blink",
  "language": {
      "displayName": "Blink Dagger",
      "lore": [
          "The fabled dagger used by the fastest assassin ever to walk the lands."
      ],
      "description": [
          "<h1>Active: Blink</h1> Teleport to a target point up to 1200 units away. <br><br>Blink Dagger cannot be used for 3.0 seconds after taking damage from an enemy hero or Roshan."
      ],
      "notes": [
          "Self-casting will cause you to teleport in the direction of your team's fountain.",
          "If you used Blink to teleport to a distance over the maximum range, you'll be teleported 4/5 of the maximum range instead."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 5136,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          0
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          15.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 2250,
      "shopTags": "teleport",
      "aliases": "blink dagger",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "blink_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"2": {
  "id": 2,
  "name": "item_blades_of_attack",
  "displayName": "Blades of Attack",
  "shortName": "blades_of_attack",
  "language": {
      "displayName": "Blades of Attack",
      "lore": [
          "The damage of these small, concealable blades should not be underestimated."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+9 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 430,
      "shopTags": "damage;tutorial",
      "aliases": "blades of attack",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "blades_of_attack_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"3": {
  "id": 3,
  "name": "item_broadsword",
  "displayName": "Broadsword",
  "shortName": "broadsword",
  "language": {
      "displayName": "Broadsword",
      "lore": [
          "The classic weapon of choice for knights, this blade is sturdy and reliable for slaying enemies."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+18 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1200,
      "shopTags": "damage",
      "aliases": "broadsword",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "broadsword_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"4": {
  "id": 4,
  "name": "item_chainmail",
  "displayName": "Chainmail",
  "shortName": "chainmail",
  "language": {
      "displayName": "Chainmail",
      "lore": [
          "A medium weave of metal chains."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+4 Armor"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 550,
      "shopTags": "armor",
      "aliases": "chainmail",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "chainmail_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"5": {
  "id": 5,
  "name": "item_claymore",
  "displayName": "Claymore",
  "shortName": "claymore",
  "language": {
      "displayName": "Claymore",
      "lore": [
          "A sword that can cut through armor, it's a commonly chosen first weapon for budding swordsmen."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+21 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1400,
      "shopTags": "damage",
      "aliases": "claymore",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "claymore_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"6": {
  "id": 6,
  "name": "item_helm_of_iron_will",
  "displayName": "Helm of Iron Will",
  "shortName": "helm_of_iron_will",
  "language": {
      "displayName": "Helm of Iron Will",
      "lore": [
          "The helmet of a legendary warrior who fell in battle."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+5 Armor",
          "+3.5 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 900,
      "shopTags": "armor;regen_health",
      "aliases": "helm of iron will",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "helm_of_iron_will_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"7": {
  "id": 7,
  "name": "item_javelin",
  "displayName": "Javelin",
  "shortName": "javelin",
  "language": {
      "displayName": "Javelin",
      "lore": [
          "A rather typical spear that can sometimes pierce through an enemy's armor when used to attack."
      ],
      "description": [
          "<h1>Passive: Pierce</h1>Grants each attack a 25% chance to pierce through evasion and deal 100 bonus magical damage."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1100,
      "shopTags": "damage",
      "aliases": "javelin",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "javelin_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"8": {
  "id": 8,
  "name": "item_mithril_hammer",
  "displayName": "Mithril Hammer",
  "shortName": "mithril_hammer",
  "language": {
      "displayName": "Mithril Hammer",
      "lore": [
          "A hammer forged of pure mithril."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+24 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1600,
      "shopTags": "damage",
      "aliases": "mithril hammer",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "mithril_hammer_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"9": {
  "id": 9,
  "name": "item_platemail",
  "displayName": "Platemail",
  "shortName": "platemail",
  "language": {
      "displayName": "Platemail",
      "lore": [
          "Thick metal plates that protect the entire upper body. Avoid dropping on feet."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+10 Armor"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1400,
      "shopTags": "armor",
      "aliases": "platemail",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "platemail_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"10": {
  "id": 10,
  "name": "item_quarterstaff",
  "displayName": "Quarterstaff",
  "shortName": "quarterstaff",
  "language": {
      "displayName": "Quarterstaff",
      "lore": [
          "A basic staff that allows you to strike quickly."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+10 Damage",
          "+10 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 875,
      "shopTags": "damage;attack_speed",
      "aliases": "quarterstaff",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "quarterstaff_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"11": {
  "id": 11,
  "name": "item_quelling_blade",
  "displayName": "Quelling Blade",
  "shortName": "quelling_blade",
  "language": {
      "displayName": "Quelling Blade",
      "lore": [
          "The axe of a fallen gnome, it allows you to effectively maneuver the forest."
      ],
      "description": [
          "<h1>Active: Chop Tree/Ward</h1> Destroy a target tree or ward. Chop cast range is increased when targeting wards.<br><br>Tree Range: %abilitycastrange%<BR>Ward Range: 450",
          "<h1>Passive: Quell</h1> Increases attack damage against non-hero units by 18 for melee heroes, and 5 for ranged."
      ],
      "notes": [
          "Effects of multiple quelling blades do not stack.",
          "Chop can be used to destroy Techies' Remote Mines."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 72,
      "unitTargetTeam": 8,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          350
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          4.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 150,
      "shopTags": "damage",
      "aliases": "qb;quelling blade",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "quelling_blade_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"12": {
  "id": 12,
  "name": "item_ring_of_protection",
  "displayName": "Ring of Protection",
  "shortName": "ring_of_protection",
  "language": {
      "displayName": "Ring of Protection",
      "lore": [
          "A glimmering ring that defends its bearer."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+2 Armor"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 150,
      "shopTags": "armor",
      "aliases": "rop;ring of protection",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ring_of_protection_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"13": {
  "id": 13,
  "name": "item_gauntlets",
  "displayName": "Gauntlets of Strength",
  "shortName": "gauntlets",
  "language": {
      "displayName": "Gauntlets of Strength",
      "lore": [
          "Studded leather gloves that add brute strength."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+3 Strength"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 145,
      "shopTags": "str",
      "aliases": "gauntlets of strength",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "gauntlets_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"14": {
  "id": 14,
  "name": "item_slippers",
  "displayName": "Slippers of Agility",
  "shortName": "slippers",
  "language": {
      "displayName": "Slippers of Agility",
      "lore": [
          "Light boots made from spider skin that tingles your senses."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+3 Agility"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 145,
      "shopTags": "agi",
      "aliases": "slippers of agility",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "slippers_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"15": {
  "id": 15,
  "name": "item_mantle",
  "displayName": "Mantle of Intelligence",
  "shortName": "mantle",
  "language": {
      "displayName": "Mantle of Intelligence",
      "lore": [
          "A beautiful sapphire mantle worn by generations of queens."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+3 Intelligence"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 145,
      "shopTags": "int",
      "aliases": "mantle of intelligence",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "mantle_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"16": {
  "id": 16,
  "name": "item_branches",
  "displayName": "Iron Branch",
  "shortName": "branches",
  "language": {
      "displayName": "Iron Branch",
      "lore": [
          "A seemingly ordinary branch, its ironlike qualities are bestowed upon the bearer."
      ],
      "description": [
          "<h1>Use: Plant Tree</h1> Targets the ground to plant a happy little tree that lasts for 20 seconds.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+1 All Attributes"
      ]
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          400
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 50,
      "shopTags": "agi;int;str",
      "aliases": "gg branch;iron branch",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "branches_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"17": {
  "id": 17,
  "name": "item_belt_of_strength",
  "displayName": "Belt of Strength",
  "shortName": "belt_of_strength",
  "language": {
      "displayName": "Belt of Strength",
      "lore": [
          "A valued accessory for improving vitality."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+6 Strength"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 450,
      "shopTags": "str",
      "aliases": "belt of strength",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "belt_of_strength_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"18": {
  "id": 18,
  "name": "item_boots_of_elves",
  "displayName": "Band of Elvenskin",
  "shortName": "boots_of_elves",
  "language": {
      "displayName": "Band of Elvenskin",
      "lore": [
          "A tensile fabric often used for its light weight and ease of movement."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+6 Agility"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 450,
      "shopTags": "agi",
      "aliases": "band of elvenskin",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "boots_of_elves_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"19": {
  "id": 19,
  "name": "item_robe",
  "displayName": "Robe of the Magi",
  "shortName": "robe",
  "language": {
      "displayName": "Robe of the Magi",
      "lore": [
          "This robe corrupts the soul of the user, but provides wisdom in return."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+6 Intelligence"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 450,
      "shopTags": "int",
      "aliases": "robe of the magi",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "robe_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"20": {
  "id": 20,
  "name": "item_circlet",
  "displayName": "Circlet",
  "shortName": "circlet",
  "language": {
      "displayName": "Circlet",
      "lore": [
          "An elegant circlet designed for human princesses."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+2 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 155,
      "shopTags": "agi;int;str",
      "aliases": "circlet",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "circlet_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"21": {
  "id": 21,
  "name": "item_ogre_axe",
  "displayName": "Ogre Axe",
  "shortName": "ogre_axe",
  "language": {
      "displayName": "Ogre Axe",
      "lore": [
          "You grow stronger just by holding it."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+10 Strength"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1000,
      "shopTags": "str",
      "aliases": "ogre club",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ogre_axe_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"22": {
  "id": 22,
  "name": "item_blade_of_alacrity",
  "displayName": "Blade of Alacrity",
  "shortName": "blade_of_alacrity",
  "language": {
      "displayName": "Blade of Alacrity",
      "lore": [
          "A long blade imbued with time magic."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+10 Agility"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1000,
      "shopTags": "agi",
      "aliases": "blade of alacrity",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "blade_of_alacrity_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"23": {
  "id": 23,
  "name": "item_staff_of_wizardry",
  "displayName": "Staff of Wizardry",
  "shortName": "staff_of_wizardry",
  "language": {
      "displayName": "Staff of Wizardry",
      "lore": [
          "A staff of magical powers passed down from the eldest mages."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+10 Intelligence"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1000,
      "shopTags": "int",
      "aliases": "staff of wizardry",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "staff_of_wizardry_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"24": {
  "id": 24,
  "name": "item_ultimate_orb",
  "displayName": "Ultimate Orb",
  "shortName": "ultimate_orb",
  "language": {
      "displayName": "Ultimate Orb",
      "lore": [
          "A mystical orb containing the essence of life."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+10 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2150,
      "shopTags": "agi;int;str",
      "aliases": "ultimate orb",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ultimate_orb_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"25": {
  "id": 25,
  "name": "item_gloves",
  "displayName": "Gloves of Haste",
  "shortName": "gloves",
  "language": {
      "displayName": "Gloves of Haste",
      "lore": [
          "A pair of magical gloves that seems to render weapons weightless."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+20 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 400,
      "shopTags": "attack_speed",
      "aliases": "gloves of haste",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "gloves_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"26": {
  "id": 26,
  "name": "item_lifesteal",
  "displayName": "Morbid Mask",
  "shortName": "lifesteal",
  "language": {
      "displayName": "Morbid Mask",
      "lore": [
          "A mask that drains the energy of those caught in its gaze."
      ],
      "description": [
          "<h1>Passive: Lifesteal</h1>Heals the attacker for 15% of attack damage dealt."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 900,
      "shopTags": "unique",
      "aliases": "morbid mask",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "lifesteal_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"27": {
  "id": 27,
  "name": "item_ring_of_regen",
  "displayName": "Ring of Regen",
  "shortName": "ring_of_regen",
  "language": {
      "displayName": "Ring of Regen",
      "lore": [
          "This ring is considered a good luck charm among the Gnomes."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+2 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 250,
      "shopTags": "regen_health",
      "aliases": "ror",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ring_of_regen_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"28": {
  "id": 28,
  "name": "item_sobi_mask",
  "displayName": "Sage's Mask",
  "shortName": "sobi_mask",
  "language": {
      "displayName": "Sage's Mask",
      "lore": [
          "A mask commonly used by mages and warlocks for various rituals."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+0.75 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 250,
      "shopTags": "regen_mana",
      "aliases": "sage's mask",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "sobi_mask_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"29": {
  "id": 29,
  "name": "item_boots",
  "displayName": "Boots of Speed",
  "shortName": "boots",
  "language": {
      "displayName": "Boots of Speed",
      "lore": [
          "Fleet footwear, increasing movement."
      ],
      "description": [
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [],
      "attributes": [
          "+15% Movement Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 500,
      "shopTags": "move_speed",
      "aliases": "boots of speed",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "boots_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": false
},
"30": {
  "id": 30,
  "name": "item_gem",
  "displayName": "Gem of True Sight",
  "shortName": "gem",
  "language": {
      "displayName": "Gem of True Sight",
      "lore": [
          "Not one thrall creature of the depths,",
          "Nor spirit bound in drowning's keep,",
          "Nor Maelrawn the Tentacular,",
          "Shall rest till seas, gem comes to sleep."
      ],
      "description": [
          "<h1>Passive: True Sight</h1> Grants the ability to see invisible units and wards to any allied vision within 900 range of its carrier. ",
          "<h1>Passive: Everlasting</h1><font color='#e03e2e'>Dropped on death, and cannot be destroyed.</font>"
      ],
      "notes": [
          "Disabled while on a courier."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          900
      ],
      "sharedCooldown": "",
      "cost": 900,
      "shopTags": "see_invis",
      "aliases": "gem of true sight",
      "quality": "component",
      "isSellable": false,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": true,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 600,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "gem_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": false
},
"31": {
  "id": 31,
  "name": "item_cloak",
  "displayName": "Cloak",
  "shortName": "cloak",
  "language": {
      "displayName": "Cloak",
      "lore": [
          "A cloak made of a magical material that works to dispel any magic cast on it."
      ],
      "description": [],
      "notes": [
          "Stacks multiplicatively with other sources of spell resistance."
      ],
      "attributes": [
          "+15% Magic Resistance"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 550,
      "shopTags": "magic_resist",
      "aliases": "cloak",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "cloak_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"32": {
  "id": 32,
  "name": "item_talisman_of_evasion",
  "displayName": "Talisman of Evasion",
  "shortName": "talisman_of_evasion",
  "language": {
      "displayName": "Talisman of Evasion",
      "lore": [
          "A necklace that allows you to anticipate enemy attacks."
      ],
      "description": [],
      "notes": [
          "Stacks diminishingly with other sources of Evasion."
      ],
      "attributes": [
          "+15% Evasion"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1400,
      "shopTags": "evasion",
      "aliases": "talisman of evasion",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "talisman_of_evasion_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"33": {
  "id": 33,
  "name": "item_cheese",
  "displayName": "Cheese",
  "shortName": "cheese",
  "language": {
      "displayName": "Cheese",
      "lore": [
          "Made from the milk of a long lost Furbolg vendor, it restores the vitality of those who taste it."
      ],
      "description": [
          "<h1>Use: Fondue</h1> Instantly restores 2500 health and 1500 mana."
      ],
      "notes": [
          "Cheese is shareable."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 2,
      "cooldown": [
          40.0
      ],
      "sharedCooldown": "",
      "cost": 1000,
      "shopTags": "",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "cheese_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"34": {
  "id": 34,
  "name": "item_magic_stick",
  "displayName": "Magic Stick",
  "shortName": "magic_stick",
  "language": {
      "displayName": "Magic Stick",
      "lore": [
          "A simple wand used to channel magic energies, it is favored by apprentice wizards and great warlocks alike."
      ],
      "description": [
          "<h1>Active: Energy Charge</h1> Instantly restores 15 health and mana per charge stored.<br><br> Max 10 charges. Gains a charge whenever a visible enemy within 1200 range uses an ability."
      ],
      "notes": [
          "Gains charges for spells cast by visible enemies in 1200 range.",
          "Certain abilities and item abilities will not add charges."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "cooldown": [
          13.0
      ],
      "sharedCooldown": "magicwand",
      "cost": 200,
      "shopTags": "regen_health;regen_mana;boost_health;boost_mana",
      "aliases": "magic stick",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": true,
      "isDisplayCharges": true,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "magic_stick_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"35": {
  "id": 35,
  "name": "item_recipe_magic_wand",
  "shortName": "recipe_magic_wand",
  "language": {
      "displayName": "Magic Wand Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 150,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"36": {
  "id": 36,
  "name": "item_magic_wand",
  "displayName": "Magic Wand",
  "shortName": "magic_wand",
  "language": {
      "displayName": "Magic Wand",
      "lore": [
          "A simple wand used to channel magic energies, it is favored by apprentice wizards and great warlocks alike."
      ],
      "description": [
          "<h1>Active: Energy Charge</h1>Instantly restores 15 health and mana per charge stored. <br><br>Max 20 charges. Gains a charge whenever a visible enemy within 1200 range uses an ability."
      ],
      "notes": [
          "Gains charges for spells cast by visible enemies in 1200 range.",
          "Certain abilities and item abilities will not add charges."
      ],
      "attributes": [
          "+3 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "cooldown": [
          13.0
      ],
      "sharedCooldown": "magicwand",
      "cost": 450,
      "shopTags": "regen_health;regen_mana;boost_health;boost_mana;int;agi;str",
      "aliases": "magic wand",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": true,
      "isDisplayCharges": true,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 34
      },
      {
          "index": 2,
          "componentId": 16
      },
      {
          "index": 3,
          "componentId": 16
      }
  ],
  "image": "magic_wand_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"37": {
  "id": 37,
  "name": "item_ghost",
  "displayName": "Ghost Scepter",
  "shortName": "ghost",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Ghost Scepter",
      "lore": [
          "Imbues the wielder with a ghostly presence, allowing them to evade physical damage."
      ],
      "description": [
          "<h1>Active: Ghost Form</h1> You enter ghost form for 4.0 seconds, becoming immune to physical damage, but are unable to attack and -40% more vulnerable to magic damage."
      ],
      "notes": [
          "Ends if you become Spell Immune, and will have no effect if you are already Spell Immune.",
          "Shares cooldown with Ethereal Blade."
      ],
      "attributes": [
          "+5 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "cooldown": [
          20.0
      ],
      "sharedCooldown": "ethereal",
      "cost": 1500,
      "shopTags": "int;agi;str;hard_to_tag",
      "aliases": "ghost scepter",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ghost_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"38": {
  "id": 38,
  "name": "item_clarity",
  "displayName": "Clarity",
  "shortName": "clarity",
  "language": {
      "displayName": "Clarity",
      "lore": [
          "Clear water that enhances the ability to meditate."
      ],
      "description": [
          "<h1>Use: Replenish</h1> Grants 4.5 mana regeneration to the target for 50 seconds.<br><br>If the unit is attacked by an enemy hero or Roshan, the effect is lost.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          250
      ],
      "castPoint": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 50,
      "shopTags": "consumable",
      "aliases": "clarity",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "clarity_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"39": {
  "id": 39,
  "name": "item_flask",
  "displayName": "Healing Salve",
  "shortName": "flask",
  "language": {
      "displayName": "Healing Salve",
      "lore": [
          "A magical salve that can quickly mend even the deepest of wounds."
      ],
      "description": [
          "<h1>Use: Salve</h1> Grants 50 health regeneration to the target for 8 seconds.<br><br>If the unit is attacked by an enemy hero or Roshan, the effect is lost.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          250
      ],
      "castPoint": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 110,
      "shopTags": "consumable",
      "aliases": "healing salve",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "flask_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"40": {
  "id": 40,
  "name": "item_dust",
  "displayName": "Dust of Appearance",
  "shortName": "dust",
  "language": {
      "displayName": "Dust of Appearance",
      "lore": [
          "One may hide visage, but never volume."
      ],
      "description": [
          "<h1>Use: Reveal</h1>Reveals and slows invisible heroes by -20% in a 1050 radius around the caster for 12 seconds."
      ],
      "notes": [
          "Places a debuff on enemy units in the area that reveals them when they are invisible."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1050
      ],
      "cooldown": [
          30.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 90,
      "shopTags": "consumable",
      "aliases": "dust of appearance",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": true,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "dust_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"41": {
  "id": 41,
  "name": "item_bottle",
  "displayName": "Bottle",
  "shortName": "bottle",
  "language": {
      "displayName": "Bottle",
      "lore": [
          "An old bottle that survived the ages, the contents placed inside become enchanted."
      ],
      "description": [
          "<h1>Active: Regenerate</h1>Consumes a charge to restore 125 health and 75 mana over 2.5 seconds. If the hero is attacked by an enemy hero or Roshan, the effect is lost.<br><br>The Bottle automatically refills at the fountain.<br><br>Hold Control to use on an allied hero.<br><br>Range: %abilitycastrange%",
          "<h1>Passive:  Store Rune</h1>Runes can be stored in the bottle for later use by right-clicking them. Unused runes will automatically activate after 2 minutes.<br><br>Using a stored rune fully refills the Bottle."
      ],
      "notes": [
          "Bottle is shareable. Stored runes cannot be shared.",
          "If Bottle does not have full charges and is placed on a courier, the courier's movement speed will be slowed by 30%."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          350
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          0.5
      ],
      "sharedCooldown": "",
      "cost": 650,
      "shopTags": "consumable",
      "aliases": "bottle",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": true,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": true,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 3,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "bottle_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"42": {
  "id": 42,
  "name": "item_ward_observer",
  "displayName": "Observer Ward",
  "shortName": "ward_observer",
  "language": {
      "displayName": "Observer Ward",
      "lore": [
          "A form of half-sentient plant, often cultivated by apprentice wizards."
      ],
      "description": [
          "<h1>Use: Plant</h1>Plants an Observer Ward, an invisible watcher that gives ground vision in a 1600 radius to your team. Lasts 6 minutes.<br><br>Hold Control to give one Observer Ward to an allied hero.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 16,
      "fightLevelRecap": 0,
      "castRange": [
          500
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          1.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "consumable",
      "aliases": "observer ward",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": true,
      "isTempestDoubleClonable": false,
      "stockMax": 4,
      "initialCharges": 1,
      "initialStock": 2,
      "stockTime": 135,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ward_observer_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"43": {
  "id": 43,
  "name": "item_ward_sentry",
  "displayName": "Sentry Ward",
  "shortName": "ward_sentry",
  "language": {
      "displayName": "Sentry Ward",
      "lore": [
          "A form of plant originally grown in the garden of a fearful king."
      ],
      "description": [
          "<h1>Use: Plant</h1> Plants a Sentry Ward, an invisible watcher that grants True Sight, the ability to see invisible enemy units and wards, to any existing allied vision within a 1000 radius.<br>Lasts 8 minutes.<br><br>Does not grant ground vision.<br>Hold Control to give one Sentry Ward to an allied hero.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 16,
      "fightLevelRecap": 0,
      "castRange": [
          500
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          1.0
      ],
      "sharedCooldown": "",
      "cost": 75,
      "shopTags": "consumable",
      "aliases": "sentry ward",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": true,
      "isTempestDoubleClonable": false,
      "stockMax": 10,
      "initialCharges": 1,
      "initialStock": 3,
      "stockTime": 90,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ward_sentry_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"44": {
  "id": 44,
  "name": "item_tango",
  "displayName": "Tango",
  "shortName": "tango",
  "language": {
      "displayName": "Tango",
      "lore": [
          "Forage to survive on the battlefield."
      ],
      "description": [
          "<h1>Use: Devour</h1> Consumes a target tree to gain 7.0 health regeneration for 16.0 seconds. Consuming a ward or Ironwood Tree doubles the heal amount.<br><br>Comes with 3 charges.  Can be used on an allied hero to give them one Tango.<br><br>Tree Range: %abilitycastrange%<br>Ward Range: 450"
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 72,
      "unitTargetTeam": 8,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          165
      ],
      "castPoint": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 90,
      "shopTags": "consumable",
      "aliases": "tango",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 3,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "tango_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"45": {
  "id": 45,
  "name": "item_courier",
  "displayName": "Animal Courier",
  "shortName": "courier",
  "language": {
      "displayName": "Animal Courier",
      "lore": [
          "Losing the courier is punishable by death."
      ],
      "description": [
          "<h1>Use: Deploy Courier</h1> Deploys a creature to carry items to and from your team's base."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 4,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 50,
      "shopTags": "consumable",
      "aliases": "donkey;chicken;animal courier",
      "quality": "consumable",
      "isSellable": false,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": true,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 36000,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "courier_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"46": {
  "id": 46,
  "name": "item_tpscroll",
  "displayName": "Town Portal Scroll",
  "shortName": "tpscroll",
  "language": {
      "displayName": "Town Portal Scroll",
      "lore": [
          "What a hero truly needs."
      ],
      "description": [
          "<h1>Use: Teleport</h1>After channeling for 3.0 seconds, teleports you to a target friendly building. <br><br>Double-click to teleport to your team's base fountain."
      ],
      "notes": [
          "If multiple heroes teleport to the same location in succession, the channeling time will be increased for each successive hero.",
          "Teleport can be prevented or canceled by Root abilities."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 604016784,
      "unitTargetType": 32,
      "unitTargetTeam": 2,
      "unitTargetFlags": 32,
      "fightLevelRecap": 0,
      "castRange": [
          0
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          80.0
      ],
      "manaCost": [
          75.0
      ],
      "channelTime": [
          3.0
      ],
      "sharedCooldown": "teleport",
      "cost": 50,
      "shopTags": "consumable;tutorial",
      "aliases": "tp;town portal scroll;teleport",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "tpscroll_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"47": {
  "id": 47,
  "name": "item_recipe_travel_boots",
  "shortName": "recipe_travel_boots",
  "language": {
      "displayName": "Boots of Travel Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2000,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"48": {
  "id": 48,
  "name": "item_travel_boots",
  "displayName": "Boots of Travel",
  "shortName": "travel_boots",
  "language": {
      "displayName": "Boots of Travel",
      "lore": [
          "Winged boots that grant omnipresence."
      ],
      "description": [
          "<h1>Active: Teleport</h1>Teleports you to an allied non-hero unit or structure. Teleporting to a unit is interrupted if the target unit dies.<br><br>Adds 10 seconds to the cooldown of Town Portal Scroll.",
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [],
      "attributes": [
          "+32% Movement Speed"
      ]
  },
  "stat": {
      "behavior": 37020,
      "unitTargetType": 5,
      "unitTargetTeam": 2,
      "unitTargetFlags": 32,
      "fightLevelRecap": 0,
      "cooldown": [
          45.0
      ],
      "manaCost": [
          75.0
      ],
      "channelTime": [
          3.0
      ],
      "sharedCooldown": "travel",
      "cost": 2500,
      "shopTags": "teleport;move_speed",
      "aliases": "bot;boots of travel;tp",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 29
      }
  ],
  "image": "travel_boots_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": false
},
"49": {
  "id": 49,
  "name": "item_recipe_phase_boots",
  "shortName": "recipe_phase_boots",
  "language": {
      "displayName": "Recipe: Phase Boots",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"50": {
  "id": 50,
  "name": "item_phase_boots",
  "displayName": "Phase Boots",
  "shortName": "phase_boots",
  "language": {
      "displayName": "Phase Boots",
      "lore": [
          "Boots that allow the wearer to travel between the ether."
      ],
      "description": [
          "<h1>Active: Phase</h1> Gives 22% increased movement speed on melee heroes, and 10 on ranged heroes, and lets you move through units and turn more quickly for 3.0 seconds.",
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [],
      "attributes": [
          "+18 Damage (MELEE)",
          "+12 Damage (RANGED)",
          "+15% Movement Speed",
          "+4 Armor"
      ]
  },
  "stat": {
      "behavior": 526340,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          8.0
      ],
      "sharedCooldown": "",
      "cost": 1480,
      "shopTags": "damage;move_speed;hard_to_tag",
      "aliases": "phase boots",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 29
      },
      {
          "index": 2,
          "componentId": 4
      },
      {
          "index": 3,
          "componentId": 2
      }
  ],
  "image": "phase_boots_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": false
},
"51": {
  "id": 51,
  "name": "item_demon_edge",
  "displayName": "Demon Edge",
  "shortName": "demon_edge",
  "language": {
      "displayName": "Demon Edge",
      "lore": [
          "One of the oldest weapons forged by the Demon-Smith Abzidian, it killed its maker when he tested its edge."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+42 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2200,
      "shopTags": "damage",
      "aliases": "demon edge",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "demon_edge_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"52": {
  "id": 52,
  "name": "item_eagle",
  "displayName": "Eaglesong",
  "shortName": "eagle",
  "language": {
      "displayName": "Eaglesong",
      "lore": [
          "Capturing the majestic call of an eagle, this mystical horn brings limitless dexterity to those who hear it."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+25 Agility"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 3200,
      "shopTags": "agi",
      "aliases": "eaglesong",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "eagle_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"53": {
  "id": 53,
  "name": "item_reaver",
  "displayName": "Reaver",
  "shortName": "reaver",
  "language": {
      "displayName": "Reaver",
      "lore": [
          "A massive axe capable of tearing whole mountains down."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+25 Strength"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 3000,
      "shopTags": "str",
      "aliases": "reaver",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "reaver_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"54": {
  "id": 54,
  "name": "item_relic",
  "displayName": "Sacred Relic",
  "shortName": "relic",
  "language": {
      "displayName": "Sacred Relic",
      "lore": [
          "An ancient weapon that often turns the tides of war."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+60 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 3800,
      "shopTags": "damage",
      "aliases": "sacred relic",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "relic_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"55": {
  "id": 55,
  "name": "item_hyperstone",
  "displayName": "Hyperstone",
  "shortName": "hyperstone",
  "language": {
      "displayName": "Hyperstone",
      "lore": [
          "A mystical, carved stone that boosts the fervor of the holder."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+55 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2000,
      "shopTags": "attack_speed",
      "aliases": "hyperstone",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "hyperstone_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"56": {
  "id": 56,
  "name": "item_ring_of_health",
  "displayName": "Ring of Health",
  "shortName": "ring_of_health",
  "language": {
      "displayName": "Ring of Health",
      "lore": [
          "A shiny ring found beneath a fat halfling's corpse."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+6 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 850,
      "shopTags": "regen_health",
      "aliases": "roh;ring of health",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ring_of_health_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"57": {
  "id": 57,
  "name": "item_void_stone",
  "displayName": "Void Stone",
  "shortName": "void_stone",
  "language": {
      "displayName": "Void Stone",
      "lore": [
          "Jewelry that was once used to channel nether realm magic, this ring pulses with energy."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+1.75 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 850,
      "shopTags": "regen_mana",
      "aliases": "void stone",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "void_stone_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"58": {
  "id": 58,
  "name": "item_mystic_staff",
  "displayName": "Mystic Staff",
  "shortName": "mystic_staff",
  "language": {
      "displayName": "Mystic Staff",
      "lore": [
          "Enigmatic staff made of only the most expensive crystals."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+25 Intelligence"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2700,
      "shopTags": "int",
      "aliases": "mystic staff",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "mystic_staff_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"59": {
  "id": 59,
  "name": "item_energy_booster",
  "displayName": "Energy Booster",
  "shortName": "energy_booster",
  "language": {
      "displayName": "Energy Booster",
      "lore": [
          "This lapis gemstone is commonly added to the collection of wizards seeking to improve their presence in combat."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+250 Mana"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 900,
      "shopTags": "mana_pool",
      "aliases": "energy booster",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "energy_booster_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"60": {
  "id": 60,
  "name": "item_point_booster",
  "displayName": "Point Booster",
  "shortName": "point_booster",
  "language": {
      "displayName": "Point Booster",
      "lore": [
          "A perfectly formed amethyst that nourishes body and mind when held."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+175 Mana",
          "+175 Health"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1200,
      "shopTags": "mana_pool;health_pool",
      "aliases": "point booster",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "point_booster_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"61": {
  "id": 61,
  "name": "item_vitality_booster",
  "displayName": "Vitality Booster",
  "shortName": "vitality_booster",
  "language": {
      "displayName": "Vitality Booster",
      "lore": [
          "A ruby gemstone that has been passed down through generations of warrior kin."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+250 Health"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1100,
      "shopTags": "health_pool",
      "aliases": "vitality booster",
      "quality": "secret_shop",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": true,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "vitality_booster_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"62": {
  "id": 62,
  "name": "item_recipe_power_treads",
  "shortName": "recipe_power_treads",
  "language": {
      "displayName": "Recipe: Power Treads",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"63": {
  "id": 63,
  "name": "item_power_treads",
  "displayName": "Power Treads",
  "shortName": "power_treads",
  "language": {
      "displayName": "Power Treads",
      "lore": [
          "A pair of tough-skinned boots that change to meet the demands of the wearer."
      ],
      "description": [
          "<h1>Active: Switch Attribute</h1>Switches between +10 Strength, +10 Agility, or +10 Intelligence.",
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [
          "Power Treads can be built using a Belt of Strength, Band of Elvenskin, or a Robe of the Magi."
      ],
      "attributes": [
          "+15% Movement Speed",
          "+10 Selected Attribute",
          "+25 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1350,
      "shopTags": "attack_speed;move_speed;int;agi;str",
      "aliases": "power treads",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 29
      },
      {
          "index": 2,
          "componentId": 25
      },
      {
          "index": 3,
          "componentId": 17
      }
  ],
  "image": "power_treads_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": false
},
"64": {
  "id": 64,
  "name": "item_recipe_hand_of_midas",
  "shortName": "recipe_hand_of_midas",
  "language": {
      "displayName": "Hand of Midas Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1850,
      "shopTags": "",
      "aliases": "hom",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"65": {
  "id": 65,
  "name": "item_hand_of_midas",
  "displayName": "Hand of Midas",
  "shortName": "hand_of_midas",
  "language": {
      "displayName": "Hand of Midas",
      "lore": [
          "Preserved through unknown magical means, the Hand of Midas is a weapon of greed, sacrificing animals to line the owner's pockets."
      ],
      "description": [
          "<h1>Active: Transmute</h1> Kills a non-hero target for 160 gold and 1.85x experience. <br><br> Cannot be used on Ancient Creeps.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [
          "The gold given is reliable gold (you do not get the normal creep bounty).",
          "Experience gained by using Transmute is not shared."
      ],
      "attributes": [
          "+40 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 32776,
      "unitTargetType": 4,
      "unitTargetTeam": 1,
      "unitTargetFlags": 5,
      "fightLevelRecap": 0,
      "castRange": [
          600
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          90.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 2250,
      "shopTags": "attack_speed;hard_to_tag",
      "aliases": "hand of midas",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 25
      }
  ],
  "image": "hand_of_midas_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"66": {
  "id": 66,
  "name": "item_recipe_oblivion_staff",
  "shortName": "recipe_oblivion_staff",
  "language": {
      "displayName": "Recipe: Oblivion Staff",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"67": {
  "id": 67,
  "name": "item_oblivion_staff",
  "displayName": "Oblivion Staff",
  "shortName": "oblivion_staff",
  "language": {
      "displayName": "Oblivion Staff",
      "lore": [
          "Deceptively hidden as an ordinary quarterstaff, it is actually very powerful, much like the Eldritch who originally possessed it."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+10 Intelligence",
          "+10 Attack Speed",
          "+10 Damage",
          "+1.25 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1575,
      "shopTags": "damage;int;attack_speed;regen_mana",
      "aliases": "oblivion staff",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 10
      },
      {
          "index": 2,
          "componentId": 28
      },
      {
          "index": 3,
          "componentId": 19
      }
  ],
  "image": "oblivion_staff_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"68": {
  "id": 68,
  "name": "item_recipe_pers",
  "shortName": "recipe_pers",
  "language": {
      "displayName": "Recipe: Perseverance",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"69": {
  "id": 69,
  "name": "item_pers",
  "displayName": "Perseverance",
  "shortName": "pers",
  "language": {
      "displayName": "Perseverance",
      "lore": [
          "A gem that grants heart to the bearer."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+6.0 HP Regeneration",
          "+2.0 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1700,
      "shopTags": "regen_health;regen_mana",
      "aliases": "perseverance",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 56
      },
      {
          "index": 2,
          "componentId": 57
      }
  ],
  "image": "pers_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"70": {
  "id": 70,
  "name": "item_recipe_poor_mans_shield",
  "shortName": "recipe_poor_mans_shield",
  "language": {
      "displayName": "Recipe: Poor Man's Shield",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"71": {
  "id": 71,
  "name": "item_poor_mans_shield",
  "displayName": "Poor Man's Shield",
  "shortName": "poor_mans_shield",
  "language": {
      "displayName": "Poor Man's Shield",
      "lore": [
          "A busted old shield that seems to block more than it should."
      ],
      "description": [
          "<h1>Passive: Damage Block</h1>Gives a 100% chance to block 20 damage from incoming attacks on melee heroes, and 10 damage on ranged.<br><br>Has a 50% chance to block damage from creeps."
      ],
      "notes": [
          "Multiple sources of damage block do not stack."
      ],
      "attributes": [
          "+7 Agility"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          6.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "agi;block",
      "aliases": "pms;poor man's shield",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "poor_mans_shield_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"72": {
  "id": 72,
  "name": "item_recipe_bracer",
  "shortName": "recipe_bracer",
  "language": {
      "displayName": "Bracer Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 210,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"73": {
  "id": 73,
  "name": "item_bracer",
  "displayName": "Bracer",
  "shortName": "bracer",
  "language": {
      "displayName": "Bracer",
      "lore": [
          "The bracer is a common choice to toughen up defenses and increase longevity."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+6 Strength",
          "+3 Agility",
          "+3 Intelligence",
          "+5% Magic Resistance"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 510,
      "shopTags": "damage;int;agi;str",
      "aliases": "bracer",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 20
      },
      {
          "index": 2,
          "componentId": 13
      }
  ],
  "image": "bracer_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"74": {
  "id": 74,
  "name": "item_recipe_wraith_band",
  "shortName": "recipe_wraith_band",
  "language": {
      "displayName": "Wraith Band Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 210,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"75": {
  "id": 75,
  "name": "item_wraith_band",
  "displayName": "Wraith Band",
  "shortName": "wraith_band",
  "language": {
      "displayName": "Wraith Band",
      "lore": [
          "A circlet with faint whispers echoing about it."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+3 Strength",
          "+6 Agility",
          "+3 Intelligence",
          "+5 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 510,
      "shopTags": "damage;int;agi;str",
      "aliases": "wraith band",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 20
      },
      {
          "index": 2,
          "componentId": 14
      }
  ],
  "image": "wraith_band_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"76": {
  "id": 76,
  "name": "item_recipe_null_talisman",
  "shortName": "recipe_null_talisman",
  "language": {
      "displayName": "Null Talisman Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 210,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"77": {
  "id": 77,
  "name": "item_null_talisman",
  "displayName": "Null Talisman",
  "shortName": "null_talisman",
  "language": {
      "displayName": "Null Talisman",
      "lore": [
          "A small gemstone attached to several chains."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+3 Strength",
          "+3 Agility",
          "+6 Intelligence",
          "+3% Spell Amplification"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 510,
      "shopTags": "damage;int;agi;str",
      "aliases": "null talisman",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 20
      },
      {
          "index": 2,
          "componentId": 15
      }
  ],
  "image": "null_talisman_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"78": {
  "id": 78,
  "name": "item_recipe_mekansm",
  "shortName": "recipe_mekansm",
  "language": {
      "displayName": "Mekansm Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 950,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"79": {
  "id": 79,
  "name": "item_mekansm",
  "displayName": "Mekansm",
  "shortName": "mekansm",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Mekansm",
      "lore": [
          "A glowing jewel formed out of assorted parts that somehow fit together perfectly."
      ],
      "description": [
          "<h1>Active: Restore</h1>Heals 275 health to allied units in a 1200 radius.",
          "<h1>Passive: Mekansm Aura</h1>Grants 2.5 health regeneration to allied units in a 1200 radius."
      ],
      "notes": [
          "Restore does not affect units that have been affected by Restore in the last 25 seconds.",
          "Multiple instances of Mekansm Aura do not stack."
      ],
      "attributes": [
          "+4 All Attributes",
          "+6 Armor"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 2,
      "castRange": [
          1200
      ],
      "cooldown": [
          65.0
      ],
      "manaCost": [
          225.0
      ],
      "sharedCooldown": "",
      "cost": 1950,
      "shopTags": "agi;int;str;armor;boost_health",
      "aliases": "mechanism;mekansm",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isAlertable": true,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 94
      },
      {
          "index": 2,
          "componentId": 4
      }
  ],
  "image": "mekansm_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"80": {
  "id": 80,
  "name": "item_recipe_vladmir",
  "shortName": "recipe_vladmir",
  "language": {
      "displayName": "Vladmir's Offering Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 450,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"81": {
  "id": 81,
  "name": "item_vladmir",
  "displayName": "Vladmir's Offering",
  "shortName": "vladmir",
  "language": {
      "displayName": "Vladmir's Offering",
      "lore": [
          "An eerie mask that is haunted with the malice of a fallen vampire."
      ],
      "description": [
          "<h1>Passive: Vladmir's Aura</h1>Grants 25 lifesteal, 1.5 mana regeneration, and 3 armor to nearby allies. <br><br>Radius: 1200"
      ],
      "notes": [],
      "attributes": [
          "+6 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "sharedCooldown": "",
      "cost": 2300,
      "shopTags": "lifesteal;damage;armor;regen_mana",
      "aliases": "vladmir's offering;vlads",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 88
      },
      {
          "index": 2,
          "componentId": 86
      },
      {
          "index": 3,
          "componentId": 26
      }
  ],
  "image": "vladmir_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"84": {
  "id": 84,
  "name": "item_flying_courier",
  "displayName": "Flying Courier",
  "shortName": "flying_courier",
  "language": {
      "displayName": "Flying Courier",
      "lore": [
          "The faithful mount upon which soar the fortunes of battle."
      ],
      "description": [
          "<h1>Use: Upgrade Courier</h1>Upgrades your team's Animal Courier to a Flying Courier, granting it swift, unobstructed movement to carry items to and from your team's base. <br><br>Requires a deployed Animal Courier."
      ],
      "notes": [],
      "attributes": []
  },
  "image": "flying_courier_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"85": {
  "id": 85,
  "name": "item_recipe_buckler",
  "shortName": "recipe_buckler",
  "language": {
      "displayName": "Buckler Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 300,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"86": {
  "id": 86,
  "name": "item_buckler",
  "displayName": "Buckler",
  "shortName": "buckler",
  "language": {
      "displayName": "Buckler",
      "lore": [
          "A powerful shield that imbues the bearer with the strength of heroes past, it is capable of protecting entire armies in battle."
      ],
      "description": [
          "<h1>Passive: Buckler Aura</h1> Grants 2 armor to allies.",
          "<h1>Toggle: Aura</h1> Deactivate to stop affecting non-hero units. <br><br>Radius: 1200"
      ],
      "notes": [],
      "attributes": [
          "+3 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2564,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "sharedCooldown": "",
      "cost": 500,
      "shopTags": "armor;boost_armor",
      "aliases": "buckler",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 12
      },
      {
          "index": 2,
          "componentId": 16
      }
  ],
  "image": "buckler_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"87": {
  "id": 87,
  "name": "item_recipe_ring_of_basilius",
  "shortName": "recipe_ring_of_basilius",
  "language": {
      "displayName": "Recipe: Ring of Basilius",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 150,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"88": {
  "id": 88,
  "name": "item_ring_of_basilius",
  "displayName": "Ring of Basilius",
  "shortName": "ring_of_basilius",
  "language": {
      "displayName": "Ring of Basilius",
      "lore": [
          "Ring given as a reward to the greatest mages."
      ],
      "description": [
          "<h1>Passive: Basilius Aura</h1>Grants 1.25 mana regeneration to allies.  <br><br>Radius: 1200"
      ],
      "notes": [],
      "attributes": [
          "+3 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "sharedCooldown": "",
      "cost": 450,
      "shopTags": "regen_mana",
      "aliases": "rob;ring of basilius",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 28
      },
      {
          "index": 2,
          "componentId": 16
      }
  ],
  "image": "ring_of_basilius_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"89": {
  "id": 89,
  "name": "item_recipe_pipe",
  "shortName": "recipe_pipe",
  "language": {
      "displayName": "Pipe of Insight Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 800,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"90": {
  "id": 90,
  "name": "item_pipe",
  "displayName": "Pipe of Insight",
  "shortName": "pipe",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Pipe of Insight",
      "lore": [
          "A powerful artifact of mysterious origin, it creates barriers against magical forces."
      ],
      "description": [
          "<h1>Active: Barrier</h1>Gives a shield that blocks 400 magic damage to all nearby allies. Lasts 12.0 seconds.<br><br>Radius: 1200",
          "<h1>Passive: Insight Aura</h1> Gives allied units 2 health regeneration and 10 magic resistance.<br><br>Radius: 1200"
      ],
      "notes": [
          "Multiple instances of Barrier do not stack.",
          "Stacks multiplicatively with other sources of magic resistance."
      ],
      "attributes": [
          "+8.0 HP Regeneration",
          "+30% Magic Resistance",
          "+3 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 2,
      "castRange": [
          1200
      ],
      "cooldown": [
          60.0
      ],
      "manaCost": [
          100.0
      ],
      "sharedCooldown": "",
      "cost": 2900,
      "shopTags": "regen_health;boost_magic_resist",
      "aliases": "pipe of insight",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isAlertable": true,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 131
      },
      {
          "index": 2,
          "componentId": 94
      }
  ],
  "image": "pipe_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"91": {
  "id": 91,
  "name": "item_recipe_urn_of_shadows",
  "shortName": "recipe_urn_of_shadows",
  "language": {
      "displayName": "Urn of Shadows Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 310,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"92": {
  "id": 92,
  "name": "item_urn_of_shadows",
  "displayName": "Urn of Shadows",
  "shortName": "urn_of_shadows",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Urn of Shadows",
      "lore": [
          "Contains the ashes of powerful demons."
      ],
      "description": [
          "<h1>Active: Soul Release</h1> Provides 30 health regeneration when cast on allies, and deals 25 damage per second when cast on enemies. <br><br>Lasts 8.0 seconds.  <br><br>Gains charges every time an enemy hero dies within 1400 units.  Only the closest Urn to the dying hero will gain a charge.<br><br>Cast Range: %abilitycastrange%"
      ],
      "notes": [
          "Empty urns gain 2 charges.",
          "If used on a hero with Soul Release already active on them, it will refresh its duration."
      ],
      "attributes": [
          "+1.5 Mana Regeneration",
          "+2 All Attributes",
          "+2 Armor"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 1,
      "unitTargetTeam": 4,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          950
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          7.0
      ],
      "sharedCooldown": "urn",
      "cost": 865,
      "shopTags": "regen_mana;str;boost_health",
      "aliases": "urn of shadows",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": true,
      "isHideCharges": false,
      "isRequiresCharges": true,
      "isDisplayCharges": true,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 28
      },
      {
          "index": 2,
          "componentId": 20
      },
      {
          "index": 3,
          "componentId": 12
      }
  ],
  "image": "urn_of_shadows_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"93": {
  "id": 93,
  "name": "item_recipe_headdress",
  "shortName": "recipe_headdress",
  "language": {
      "displayName": "Headdress Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 150,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"94": {
  "id": 94,
  "name": "item_headdress",
  "displayName": "Headdress",
  "shortName": "headdress",
  "language": {
      "displayName": "Headdress",
      "lore": [
          "Creates a soothing aura that restores allies in battle."
      ],
      "description": [
          "<h1>Passive: Regeneration Aura</h1>Grants 2 health regeneration to allies.<br><br>Radius: 1200"
      ],
      "notes": [],
      "attributes": [
          "+3 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "sharedCooldown": "",
      "cost": 450,
      "shopTags": "str;int;agi;regen_health",
      "aliases": "headdress",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 27
      },
      {
          "index": 2,
          "componentId": 16
      }
  ],
  "image": "headdress_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"95": {
  "id": 95,
  "name": "item_recipe_sheepstick",
  "shortName": "recipe_sheepstick",
  "language": {
      "displayName": "Recipe: Scythe of Vyse",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"96": {
  "id": 96,
  "name": "item_sheepstick",
  "displayName": "Scythe of Vyse",
  "shortName": "sheepstick",
  "language": {
      "displayName": "Scythe of Vyse",
      "lore": [
          "The most guarded relic among the cult of Vyse, it is the most coveted weapon among magi."
      ],
      "description": [
          "<h1>Active: Hex</h1>Turns a target unit into a harmless critter for 3.5 seconds. The target has a base movement speed of 140 and will be silenced, muted, and disarmed.<br>Instantly destroys illusions.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [
          "The target will have a base movement speed of %sheep_movement_speed%, but buffs granting maximum movement speed won't be disabled."
      ],
      "attributes": [
          "+10 Strength",
          "+10 Agility",
          "+35 Intelligence",
          "+9.0 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 2,
      "castRange": [
          800
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          22.0
      ],
      "manaCost": [
          250.0
      ],
      "sharedCooldown": "",
      "cost": 5700,
      "shopTags": "int;regen_mana;agi;hard_to_tag",
      "aliases": "hex;sheepstick;scythe of vyse",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 58
      },
      {
          "index": 2,
          "componentId": 24
      },
      {
          "index": 3,
          "componentId": 57
      }
  ],
  "image": "sheepstick_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"97": {
  "id": 97,
  "name": "item_recipe_orchid",
  "shortName": "recipe_orchid",
  "language": {
      "displayName": "Orchid Malevolence Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 775,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"98": {
  "id": 98,
  "name": "item_orchid",
  "displayName": "Orchid Malevolence",
  "shortName": "orchid",
  "language": {
      "displayName": "Orchid Malevolence",
      "lore": [
          "A garnet rod constructed from the essence of a fire demon."
      ],
      "description": [
          "<h1>Active: Soul Burn</h1>Silences the target unit for 5 seconds. At the end of the silence, 30% of the damage received while silenced is inflicted as bonus magical damage.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+25 Intelligence",
          "+30 Attack Speed",
          "+30 Damage",
          "+5.5 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          900
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          18.0
      ],
      "manaCost": [
          100.0
      ],
      "sharedCooldown": "orchid",
      "cost": 3925,
      "shopTags": "int;attack_speed;damage;regen_mana;damage;hard_to_tag",
      "aliases": "orchid malevolence;silence",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 67
      },
      {
          "index": 2,
          "componentId": 67
      }
  ],
  "image": "orchid_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"99": {
  "id": 99,
  "name": "item_recipe_cyclone",
  "shortName": "recipe_cyclone",
  "language": {
      "displayName": "Eul's Scepter Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 650,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"100": {
  "id": 100,
  "name": "item_cyclone",
  "displayName": "Eul's Scepter of Divinity",
  "shortName": "cyclone",
  "language": {
      "displayName": "Eul's Scepter of Divinity",
      "lore": [
          "A mysterious scepter passed down through the ages, its disruptive winds can be used for good or evil."
      ],
      "description": [
          "<h1>Active: Cyclone</h1> Sweeps a target unit up into a cyclone, making them invulnerable for 2.5 seconds. Cyclone can only be cast on enemy units or yourself.<br><br>Enemy units take 50 magical damage upon landing.<br><br>Range: %abilitycastrange%<br>Dispel Type: <span class=\\\"GameplayValues GameplayVariable\\\">Basic Dispel</span>"
      ],
      "notes": [
          "You cannot cyclone allies.",
          "Cyclones cast on yourself go through spell immunity.",
          "Cyclone can purge some buffs and debuffs."
      ],
      "attributes": [
          "+10 Intelligence",
          "+5.0 Mana Regeneration",
          "+20 Movement Speed"
      ]
  },
  "stat": {
      "behavior": 32776,
      "unitTargetType": 8,
      "unitTargetTeam": 8,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          575
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          23.0
      ],
      "manaCost": [
          175.0
      ],
      "sharedCooldown": "",
      "cost": 2750,
      "shopTags": "int;regen_mana;move_speed;hard_to_tag",
      "aliases": "cyclone euls;eul's scepter of divinity",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 23
      },
      {
          "index": 2,
          "componentId": 57
      },
      {
          "index": 3,
          "componentId": 244
      }
  ],
  "image": "cyclone_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"101": {
  "id": 101,
  "name": "item_recipe_force_staff",
  "shortName": "recipe_force_staff",
  "language": {
      "displayName": "Force Staff Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1000,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"102": {
  "id": 102,
  "name": "item_force_staff",
  "displayName": "Force Staff",
  "shortName": "force_staff",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Force Staff",
      "lore": [
          "Allows you to manipulate others, for good or evil."
      ],
      "description": [
          "<h1>Active: Force</h1>Pushes any target unit 600 units in the direction it is facing.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [
          "Self-cast will cause you to use Force on yourself.",
          "Force Staff doesn't interrupt the target's actions.",
          "Will not work on a unit inside Chronosphere, Duel, or Black Hole."
      ],
      "attributes": [
          "+10 Intelligence",
          "+2.5 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 32776,
      "unitTargetType": 11,
      "unitTargetTeam": 12,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          750
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          23.0
      ],
      "manaCost": [
          100.0
      ],
      "sharedCooldown": "force",
      "cost": 2250,
      "shopTags": "int;damage;attack_speed;hard_to_tag",
      "aliases": "fs;force staff",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 23
      },
      {
          "index": 2,
          "componentId": 27
      }
  ],
  "image": "force_staff_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"103": {
  "id": 103,
  "name": "item_recipe_dagon",
  "shortName": "recipe_dagon",
  "language": {
      "displayName": "Dagon Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1250,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"104": {
  "id": 104,
  "name": "item_dagon",
  "displayName": "Dagon",
  "shortName": "dagon",
  "language": {
      "displayName": "Dagon",
      "lore": [
          "A lesser wand that grows in power the longer it is used, it brings magic to the fingertips of the user."
      ],
      "description": [
          "<h1>Active: Energy Burst</h1> Emits a powerful burst of magical damage upon a targeted enemy unit. Upgradable.<br><br>Damage: 400 / 500 / 600 / 700 / 800<br>Range: %abilitycastrange%<br>Mana Cost: 120 / 140 / 160 / 180 / 200"
      ],
      "notes": [
          "Instantly kills illusions."
      ],
      "attributes": [
          "+13 / 16 / 19 / 22 / 25 Intelligence",
          "+5 All Attributes"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          600,
          650,
          700,
          750,
          800
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          35.0,
          30.0,
          25.0,
          20.0,
          15.0
      ],
      "manaCost": [
          120.0,
          140.0,
          160.0,
          180.0,
          200.0
      ],
      "sharedCooldown": "dagon",
      "cost": 2700,
      "shopTags": "damage;int;str;agi;hard_to_tag",
      "aliases": "dagon 1",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 23
      },
      {
          "index": 2,
          "componentId": 261
      }
  ],
  "image": "dagon_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"105": {
  "id": 105,
  "name": "item_recipe_necronomicon",
  "shortName": "recipe_necronomicon",
  "language": {
      "displayName": "Necronomicon Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1300,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"106": {
  "id": 106,
  "name": "item_necronomicon",
  "displayName": "Necronomicon",
  "shortName": "necronomicon",
  "language": {
      "displayName": "Necronomicon",
      "lore": [
          "Considered the ultimate in necromancy and demonology, a powerful malefic force is locked within its pages."
      ],
      "description": [
          "<h1>Active: Demonic Summoning</h1>Summons a Warrior and an Archer to fight for you for 60 seconds.<br><br><h1>Warrior:</h1>Burns mana every hit, and deals magical damage to whoever kills it.  Gains True Sight at level 3.<br>Health: 700 / 800 / 900<br>Damage: 75 / 100 / 125<br>Mana Break Damage: 30 / 40 / 50<br>Last Will Damage: 600 / 700 / 800<br><br><h1>Archer:</h1>Has a passive movement and attack speed aura. Gains Purge at Level 3.<br>Health: 700 / 800 / 900<br>Damage: 60 / 90 / 120<br>Aura Move Speed: 5 / 7 / 9<br>Aura Attack Speed: 5 / 7 / 9<br>Aura Radius: 1200"
      ],
      "notes": [],
      "attributes": [
          "+10 / 15 / 20 Strength",
          "+2 / 2.5 / 3.0 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castPoint": [
          0.0
      ],
      "cooldown": [
          90.0
      ],
      "manaCost": [
          50.0
      ],
      "sharedCooldown": "necronomicon",
      "cost": 2250,
      "shopTags": "int;str;hard_to_tag;see_invis",
      "aliases": "necronomicon 1",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 28
      },
      {
          "index": 2,
          "componentId": 28
      },
      {
          "index": 3,
          "componentId": 17
      }
  ],
  "image": "necronomicon_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"107": {
  "id": 107,
  "name": "item_recipe_ultimate_scepter",
  "shortName": "recipe_ultimate_scepter",
  "language": {
      "displayName": "Recipe: Aghanim's Scepter",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"108": {
  "id": 108,
  "name": "item_ultimate_scepter",
  "displayName": "Aghanim's Scepter",
  "shortName": "ultimate_scepter",
  "language": {
      "displayName": "Aghanim's Scepter",
      "lore": [
          "The scepter of a wizard with demigod-like powers."
      ],
      "description": [
          "<h1>Passive: Ability Upgrade</h1>Upgrades the ultimate, and some abilities, of all heroes."
      ],
      "notes": [],
      "attributes": [
          "+10 All Attributes",
          "+175 Health",
          "+175 Mana"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 16,
      "fightLevelRecap": 0,
      "castRange": [
          600
      ],
      "sharedCooldown": "",
      "cost": 4200,
      "shopTags": "int;str;agi;mana_pool;health_pool;hard_to_tag",
      "aliases": "ags;ultimate;aghanim's scepter;ags",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 60
      },
      {
          "index": 2,
          "componentId": 23
      },
      {
          "index": 3,
          "componentId": 21
      },
      {
          "index": 4,
          "componentId": 22
      }
  ],
  "image": "ultimate_scepter_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"109": {
  "id": 109,
  "name": "item_recipe_refresher",
  "shortName": "recipe_refresher",
  "language": {
      "displayName": "Refresher Orb Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 2,
      "sharedCooldown": "",
      "cost": 1700,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"110": {
  "id": 110,
  "name": "item_refresher",
  "displayName": "Refresher Orb",
  "shortName": "refresher",
  "language": {
      "displayName": "Refresher Orb",
      "lore": [
          "A powerful artifact created for wizards."
      ],
      "description": [
          "<h1>Active: Reset Cooldowns</h1>Resets the cooldowns of all your items and abilities."
      ],
      "notes": [],
      "attributes": [
          "+13 HP Regeneration",
          "+12.0 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castPoint": [
          0.0
      ],
      "cooldown": [
          195.0
      ],
      "manaCost": [
          375.0
      ],
      "sharedCooldown": "",
      "cost": 5100,
      "shopTags": "regen_health;regen_mana;hard_to_tag",
      "aliases": "refresher orb",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 69
      },
      {
          "index": 2,
          "componentId": 69
      }
  ],
  "image": "refresher_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"111": {
  "id": 111,
  "name": "item_recipe_assault",
  "shortName": "recipe_assault",
  "language": {
      "displayName": "Assault Cuirass Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1300,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"112": {
  "id": 112,
  "name": "item_assault",
  "displayName": "Assault Cuirass",
  "shortName": "assault",
  "language": {
      "displayName": "Assault Cuirass",
      "lore": [
          "Forged in the depths of the nether reaches, this hellish mail provides an army with increased armor and attack speed."
      ],
      "description": [
          "<h1>Passive: Assault Aura</h1> Grants 25 attack speed and 5 armor to nearby allied units and structures, and decreases nearby enemy unit and structure armor by -5.<br><br>Radius: 1200"
      ],
      "notes": [],
      "attributes": [
          "+30 Attack Speed",
          "+10 Armor",
          "+5 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "sharedCooldown": "",
      "cost": 5200,
      "shopTags": "attack_speed;armor;hard_to_tag",
      "aliases": "ac;assault cuirass",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 9
      },
      {
          "index": 2,
          "componentId": 55
      },
      {
          "index": 3,
          "componentId": 86
      }
  ],
  "image": "assault_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"113": {
  "id": 113,
  "name": "item_recipe_heart",
  "shortName": "recipe_heart",
  "language": {
      "displayName": "Heart of Tarrasque Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 400,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"114": {
  "id": 114,
  "name": "item_heart",
  "displayName": "Heart of Tarrasque",
  "shortName": "heart",
  "language": {
      "displayName": "Heart of Tarrasque",
      "lore": [
          "Preserved heart of an extinct monster, it bolsters the bearer's fortitude."
      ],
      "description": [
          "<h1>Passive: Health Regeneration</h1>Restores 5 of max health per second. <br><br>If damage is taken from an enemy hero or Roshan, this ability is disabled for 5 seconds for melee heroes, or 7 seconds for ranged heroes."
      ],
      "notes": [],
      "attributes": [
          "+45 Strength",
          "+400 Health",
          "+50% Health Regen Amplification",
          "+10 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          7.0
      ],
      "sharedCooldown": "",
      "cost": 5200,
      "shopTags": "str;regen_health;health_pool",
      "aliases": "hot;heart of tarrasque",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 279
      },
      {
          "index": 2,
          "componentId": 61
      },
      {
          "index": 3,
          "componentId": 53
      }
  ],
  "image": "heart_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"115": {
  "id": 115,
  "name": "item_recipe_black_king_bar",
  "shortName": "recipe_black_king_bar",
  "language": {
      "displayName": "Black King Bar Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1450,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"116": {
  "id": 116,
  "name": "item_black_king_bar",
  "displayName": "Black King Bar",
  "shortName": "black_king_bar",
  "language": {
      "displayName": "Black King Bar",
      "lore": [
          "A powerful staff imbued with the strength of giants."
      ],
      "description": [
          "<h1>Active: Avatar</h1> Grants Spell Immunity.  Duration decreases with each use. <br><br>Duration: 10.0 / 9.0 / 8.0 / 7.0 / 6.0 / 5.0 <br>Dispel Type: <span class=\\\"GameplayValues GameplayVariable\\\">Basic Dispel</span>"
      ],
      "notes": [
          "Purchasing another Black King Bar will not reset its immunity duration.",
          "Using Black King Bar may remove some positive buffs."
      ],
      "attributes": [
          "+10 Strength",
          "+24 Damage"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 2,
      "cooldown": [
          70.0
      ],
      "sharedCooldown": "",
      "cost": 4050,
      "shopTags": "str;damage;hard_to_tag",
      "aliases": "bkb;black king bar",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 21
      },
      {
          "index": 2,
          "componentId": 8
      }
  ],
  "image": "black_king_bar_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"117": {
  "id": 117,
  "name": "item_aegis",
  "displayName": "Aegis of the Immortal",
  "shortName": "aegis",
  "language": {
      "displayName": "Aegis of the Immortal",
      "lore": [
          "The Immortal was said to own a shield that protected him from death itself."
      ],
      "description": [
          "<h1>Passive: Reincarnation</h1> Brings you to life with full health and mana 5.0 seconds after you die, at the location where you died. <br><br>Reincarnation must be used within 5 minutes or Aegis of the Immortal disappears. If it expires, it will heal you over 5 seconds (dispels on damage)."
      ],
      "notes": [
          "Pronounced as ayy jis."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 2,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "quality": "artifact",
      "isSellable": false,
      "isDroppable": false,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "aegis_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"118": {
  "id": 118,
  "name": "item_recipe_shivas_guard",
  "shortName": "recipe_shivas_guard",
  "language": {
      "displayName": "Shiva's Guard Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 650,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"119": {
  "id": 119,
  "name": "item_shivas_guard",
  "displayName": "Shiva's Guard",
  "shortName": "shivas_guard",
  "language": {
      "displayName": "Shiva's Guard",
      "lore": [
          "Said to have belonged to a goddess, today it retains much of its former power."
      ],
      "description": [
          "<h1>Active: Arctic Blast</h1> Emits a freezing wave that deals 200 magical damage to enemies and slows their movement by -40 for 4.0 seconds.<br><br>Radius: 900",
          "<h1>Passive: Freezing Aura</h1> Reduces the attack speed of all enemies by -45. <br><br>Radius: 1200"
      ],
      "notes": [
          "The wave extends at a speed of %blast_speed% to a max size of %blast_radius%.",
          "The Arctic Blast follows its caster.",
          "Multiple instances of Freezing Aura do not stack."
      ],
      "attributes": [
          "+30 Intelligence",
          "+15 Armor"
      ]
  },
  "stat": {
      "behavior": 526340,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          900
      ],
      "cooldown": [
          30.0
      ],
      "manaCost": [
          100.0
      ],
      "sharedCooldown": "",
      "cost": 4750,
      "shopTags": "int;armor;hard_to_tag",
      "aliases": "shiva's guard;shivas",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 9
      },
      {
          "index": 2,
          "componentId": 58
      }
  ],
  "image": "shivas_guard_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"120": {
  "id": 120,
  "name": "item_recipe_bloodstone",
  "shortName": "recipe_bloodstone",
  "language": {
      "displayName": "Bloodstone Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"121": {
  "id": 121,
  "name": "item_bloodstone",
  "displayName": "Bloodstone",
  "shortName": "bloodstone",
  "language": {
      "displayName": "Bloodstone",
      "lore": [
          "The Bloodstone's bright ruby color is unmistakable on the battlefield, as the owner seems to have infinite vitality and spirit."
      ],
      "description": [
          "<h1>Active: Bloodpact</h1> Converts 30% of your max mana to health regeneration over 2.0 seconds.",
          " <h1>Passive: Mana Battery</h1>Each charge provides 0.2 MP regen and 0.35% Spell Amp. Nearby kills provide 1 charges. Dying causes you to lose 3 charges. Starts with 14 charges. Provides +100 Mana Regen Amplification."
      ],
      "notes": [],
      "attributes": [
          "+425 Health",
          "+425 Mana",
          "+16 Intelligence",
          "+8% Base Spell Amplification",
          "+12% Manacost and Manaloss Reduction"
      ]
  },
  "stat": {
      "behavior": 4,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 2,
      "castPoint": [
          0.0
      ],
      "cooldown": [
          250.0
      ],
      "sharedCooldown": "",
      "cost": 5250,
      "shopTags": "regen_health;regen_mana;mana_pool;health_pool",
      "aliases": "bs;bloodstone",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": true,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 14,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 259
      },
      {
          "index": 2,
          "componentId": 129
      }
  ],
  "image": "bloodstone_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"122": {
  "id": 122,
  "name": "item_recipe_sphere",
  "shortName": "recipe_sphere",
  "language": {
      "displayName": "Linken's Sphere Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1200,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"123": {
  "id": 123,
  "name": "item_sphere",
  "displayName": "Linken's Sphere",
  "shortName": "sphere",
  "language": {
      "displayName": "Linken's Sphere",
      "lore": [
          "This magical sphere once protected one of the most famous heroes in history."
      ],
      "description": [
          "<h1>Passive: Spellblock</h1>Blocks most targeted spells once every 12.0 seconds.",
          "<h1>Active: Transfer Spellblock</h1>Temporarily removes Spellblock from the item's owner and transfers it to an allied unit for 12.0 seconds.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+14 All Attributes",
          "+7 HP Regeneration",
          "+5.0 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          700
      ],
      "cooldown": [
          12.0
      ],
      "sharedCooldown": "",
      "cost": 5050,
      "shopTags": "regen_health;regen_mana;str;agi;int;hard_to_tag",
      "aliases": "ls;linken's sphere",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 24
      },
      {
          "index": 2,
          "componentId": 69
      }
  ],
  "image": "sphere_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"124": {
  "id": 124,
  "name": "item_recipe_vanguard",
  "shortName": "recipe_vanguard",
  "language": {
      "displayName": "Recipe: Vanguard",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 200,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"125": {
  "id": 125,
  "name": "item_vanguard",
  "displayName": "Vanguard",
  "shortName": "vanguard",
  "language": {
      "displayName": "Vanguard",
      "lore": [
          "A powerful shield that defends its wielder from even the most vicious of attacks."
      ],
      "description": [
          "<h1>Passive: Damage Block</h1> Grants a 50% chance to block 70 damage from incoming attacks on melee heroes, and 35 damage on ranged."
      ],
      "notes": [
          "Multiple sources of damage block do not stack."
      ],
      "attributes": [
          "+250 Health",
          "+7.0 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2150,
      "shopTags": "regen_health;block;health_pool",
      "aliases": "vg;vanguard",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 56
      },
      {
          "index": 2,
          "componentId": 61
      }
  ],
  "image": "vanguard_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"126": {
  "id": 126,
  "name": "item_recipe_blade_mail",
  "shortName": "recipe_blade_mail",
  "language": {
      "displayName": "Recipe: Blade Mail",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"127": {
  "id": 127,
  "name": "item_blade_mail",
  "displayName": "Blade Mail",
  "shortName": "blade_mail",
  "language": {
      "displayName": "Blade Mail",
      "lore": [
          "A razor-sharp coat of mail, it is the choice of selfless martyrs in combat."
      ],
      "description": [
          "<h1>Active: Damage Return</h1>For 4.5 seconds, duplicates any damage taken back to the unit that dealt the damage."
      ],
      "notes": [
          "Damage Return is calculated before any kind of reduction.",
          "Damage Return doesn't reflect damage from other forms of Blade Mail.",
          "Returned damage type is the same as it was received."
      ],
      "attributes": [
          "+22 Damage",
          "+6 Armor",
          "+10 Intelligence"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          25.0
      ],
      "manaCost": [
          25.0
      ],
      "sharedCooldown": "",
      "cost": 2200,
      "shopTags": "damage;armor;int;hard_to_tag",
      "aliases": "bm;blade mail;blademail",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 3
      },
      {
          "index": 2,
          "componentId": 4
      },
      {
          "index": 3,
          "componentId": 19
      }
  ],
  "image": "blade_mail_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"128": {
  "id": 128,
  "name": "item_recipe_soul_booster",
  "shortName": "recipe_soul_booster",
  "language": {
      "displayName": "Recipe: Soul Booster",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"129": {
  "id": 129,
  "name": "item_soul_booster",
  "displayName": "Soul Booster",
  "shortName": "soul_booster",
  "language": {
      "displayName": "Soul Booster",
      "lore": [
          "Regain lost courage."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+425 Health",
          "+425 Mana"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 3200,
      "shopTags": "regen_health;regen_mana;health_pool;mana_pool",
      "aliases": "soul booster",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 61
      },
      {
          "index": 2,
          "componentId": 59
      },
      {
          "index": 3,
          "componentId": 60
      }
  ],
  "image": "soul_booster_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"130": {
  "id": 130,
  "name": "item_recipe_hood_of_defiance",
  "shortName": "recipe_hood_of_defiance",
  "language": {
      "displayName": "Recipe: Hood of Defiance",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"131": {
  "id": 131,
  "name": "item_hood_of_defiance",
  "displayName": "Hood of Defiance",
  "shortName": "hood_of_defiance",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Hood of Defiance",
      "lore": [
          "A furred, magic resistant headpiece that is feared by wizards."
      ],
      "description": [
          "<h1>Active: Barrier</h1> Creates a spell shield that absorbs up to 325 magical damage.  Lasts 12.0 seconds."
      ],
      "notes": [
          "Stacks multiplicatively with other sources of spell resistance."
      ],
      "attributes": [
          "+25% Magic Resistance",
          "+8.0 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          60.0
      ],
      "manaCost": [
          75.0
      ],
      "sharedCooldown": "",
      "cost": 1650,
      "shopTags": "regen_health;magic_resist",
      "aliases": "hood of defiance",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 56
      },
      {
          "index": 2,
          "componentId": 31
      },
      {
          "index": 3,
          "componentId": 27
      }
  ],
  "image": "hood_of_defiance_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"132": {
  "id": 132,
  "name": "item_recipe_rapier",
  "shortName": "recipe_rapier",
  "language": {
      "displayName": "Recipe: Divine Rapier",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"133": {
  "id": 133,
  "name": "item_rapier",
  "displayName": "Divine Rapier",
  "shortName": "rapier",
  "language": {
      "displayName": "Divine Rapier",
      "lore": [
          "So powerful, it cannot have a single owner."
      ],
      "description": [
          "<h1>Passive: Everlasting</h1> <font color='#e03e2e'>Dropped on death, and cannot be destroyed.</font> <br><br>Becomes unusable if picked up by an ally of its owner until it is returned to its owner. It is immediately usable by anybody if an enemy of the owner picks it up and is killed. A dropped Rapier cannot be picked up by a courier. ",
          "",
          " <h1> Passive: True Strike</h1> Your attacks cannot miss."
      ],
      "notes": [
          "If Divine Rapier is dropped and picked up by an enemy of its original owner, it cannot be dropped again except by death."
      ],
      "attributes": [
          "+330 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 6000,
      "shopTags": "damage",
      "aliases": "divine rapier",
      "quality": "epic",
      "isSellable": false,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 54
      },
      {
          "index": 2,
          "componentId": 51
      }
  ],
  "image": "rapier_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"134": {
  "id": 134,
  "name": "item_recipe_monkey_king_bar",
  "shortName": "recipe_monkey_king_bar",
  "language": {
      "displayName": "Recipe: Monkey King Bar",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"135": {
  "id": 135,
  "name": "item_monkey_king_bar",
  "displayName": "Monkey King Bar",
  "shortName": "monkey_king_bar",
  "language": {
      "displayName": "Monkey King Bar",
      "lore": [
          "A powerful staff used by a master warrior."
      ],
      "description": [
          "<h1>Passive: Pierce</h1>Grants each attack a 75% chance to pierce through evasion and deal 100 bonus magical damage."
      ],
      "notes": [],
      "attributes": [
          "+52 Damage",
          "+10 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 4175,
      "shopTags": "damage;attack_speed;hard_to_tag",
      "aliases": "mkb;monkey king bar",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 51
      },
      {
          "index": 2,
          "componentId": 7
      },
      {
          "index": 3,
          "componentId": 10
      }
  ],
  "image": "monkey_king_bar_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"136": {
  "id": 136,
  "name": "item_recipe_radiance",
  "shortName": "recipe_radiance",
  "language": {
      "displayName": "Radiance Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1350,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"137": {
  "id": 137,
  "name": "item_radiance",
  "displayName": "Radiance",
  "shortName": "radiance",
  "language": {
      "displayName": "Radiance",
      "lore": [
          "A divine weapon that causes damage and a bright burning effect that lays waste to nearby enemies."
      ],
      "description": [
          "<h1>Toggle: Burn</h1> When active, scorches enemies for 60 magical damage per second, and causes them to miss 17 of their attacks. Illusions deal 35 magical damage per second.<br><br>Radius: 700"
      ],
      "notes": [],
      "attributes": [
          "+65 Damage"
      ]
  },
  "stat": {
      "behavior": 516,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          700
      ],
      "sharedCooldown": "",
      "cost": 5150,
      "shopTags": "damage",
      "aliases": "radiance",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 54
      }
  ],
  "image": "radiance_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"138": {
  "id": 138,
  "name": "item_recipe_butterfly",
  "shortName": "recipe_butterfly",
  "language": {
      "displayName": "Recipe: Butterfly",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"139": {
  "id": 139,
  "name": "item_butterfly",
  "displayName": "Butterfly",
  "shortName": "butterfly",
  "language": {
      "displayName": "Butterfly",
      "lore": [
          "Only the mightiest and most experienced of warriors can wield the Butterfly, but it provides incredible dexterity in combat."
      ],
      "description": [],
      "notes": [
          "Stacks diminishingly with other sources of Evasion."
      ],
      "attributes": [
          "+35 Agility",
          "+25 Damage",
          "+35% Evasion",
          "+30 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 5475,
      "shopTags": "agi;damage;evasion;attack_speed",
      "aliases": "bfly;butterfly",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 52
      },
      {
          "index": 2,
          "componentId": 32
      },
      {
          "index": 3,
          "componentId": 10
      }
  ],
  "image": "butterfly_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"140": {
  "id": 140,
  "name": "item_recipe_greater_crit",
  "shortName": "recipe_greater_crit",
  "language": {
      "displayName": "Daedalus Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1000,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"141": {
  "id": 141,
  "name": "item_greater_crit",
  "displayName": "Daedalus",
  "shortName": "greater_crit",
  "language": {
      "displayName": "Daedalus",
      "lore": [
          "A weapon of incredible power that is difficult for even the strongest of warriors to control."
      ],
      "description": [
          "<h1>Passive: Critical Strike</h1>Grants each attack a 30% chance to deal 235% damage."
      ],
      "notes": [
          "Critical Strike does not work against buildings."
      ],
      "attributes": [
          "+80 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 5330,
      "shopTags": "damage;crit",
      "aliases": "daedalus",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 149
      },
      {
          "index": 2,
          "componentId": 51
      }
  ],
  "image": "greater_crit_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"142": {
  "id": 142,
  "name": "item_recipe_basher",
  "shortName": "recipe_basher",
  "language": {
      "displayName": "Skull Basher Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 900,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"143": {
  "id": 143,
  "name": "item_basher",
  "displayName": "Skull Basher",
  "shortName": "basher",
  "language": {
      "displayName": "Skull Basher",
      "lore": [
          "A feared weapon in the right hands, this maul's ability to shatter the defenses of its opponents should not be underestimated."
      ],
      "description": [
          "<h1>Passive: Bash</h1> Grants melee heroes a 25% chance on hit to stun the target for 1.5 seconds and deal 100 bonus physical damage.  Bash chance for ranged heroes is 10%."
      ],
      "notes": [
          "Does not stack with other sources of Bash, however it will stack with Mini-Bash.",
          "The following heroes cannot trigger Bash on this item: Spirit Breaker, Faceless Void, and Slardar."
      ],
      "attributes": [
          "+25 Damage",
          "+10 Strength"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          2.3
      ],
      "sharedCooldown": "",
      "cost": 2950,
      "shopTags": "damage;str;hard_to_tag",
      "aliases": "skull basher",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 8
      },
      {
          "index": 2,
          "componentId": 17
      }
  ],
  "image": "basher_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"144": {
  "id": 144,
  "name": "item_recipe_bfury",
  "shortName": "recipe_bfury",
  "language": {
      "displayName": "Battle Fury Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 300,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"145": {
  "id": 145,
  "name": "item_bfury",
  "displayName": "Battle Fury",
  "shortName": "bfury",
  "language": {
      "displayName": "Battle Fury",
      "lore": [
          "The bearer of this mighty axe gains the ability to cut down swaths of enemies at once."
      ],
      "description": [
          "<h1>Active: Chop Tree/Ward</h1> Destroy a target tree or ward.<br><br>Tree Cast Range: %abilitycastrange%<br>Ward Cast Range: 450 ",
          "<h1>Passive: Quell</h1> Increases attack damage against non-hero units by 18 for melee heroes, and 5 for ranged. ",
          "<h1>Passive: Cleave</h1> Deals 60% of attack damage as physical damage in a cone up to 650 around the target. (Melee Only)"
      ],
      "notes": [
          "Cleave damage is reduced by armor type but not by armor value.",
          "Cleave damage goes through spell immunity.",
          "If multiple sources of Cleave are present, each Cleave's damage is applied separately.",
          "Chop can be used to destroy Techies' Remote Mines."
      ],
      "attributes": [
          "+54 Damage",
          "+7.5 HP Regeneration",
          "+3.75 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 72,
      "unitTargetTeam": 8,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          350
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          4.0
      ],
      "sharedCooldown": "",
      "cost": 4350,
      "shopTags": "damage;health_regen;mana_regen;hard_to_tag",
      "aliases": "bf;battle fury",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 51
      },
      {
          "index": 2,
          "componentId": 69
      },
      {
          "index": 3,
          "componentId": 11
      }
  ],
  "image": "bfury_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"146": {
  "id": 146,
  "name": "item_recipe_manta",
  "shortName": "recipe_manta",
  "language": {
      "displayName": "Manta Style Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "sharedCooldown": "",
      "cost": 500,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"147": {
  "id": 147,
  "name": "item_manta",
  "displayName": "Manta Style",
  "shortName": "manta",
  "language": {
      "displayName": "Manta Style",
      "lore": [
          "An axe made of reflective materials that causes confusion amongst enemy ranks."
      ],
      "description": [
          "<h1>Active: Mirror Image</h1>Creates 2 images of your hero that last 20 seconds. <br><br>Melee images deal 33 damage and take 350% damage, while Ranged images deal 28 and take 400 damage. <br><br>Cooldown increased by 15 seconds on ranged heroes.<br><br>Dispel Type: <span class=\\\"GameplayValues GameplayVariable\\\">Basic Dispel</span>"
      ],
      "notes": [
          "Has a %invuln_duration% second cast time during which you are invulnerable.",
          "Many effects are removed upon using Manta.",
          "Yasha based movement speed bonuses from multiple items do not stack."
      ],
      "attributes": [
          "+26 Agility",
          "+10 Strength",
          "+10 Intelligence",
          "+12 Attack Speed",
          "+20 Movement Speed"
      ]
  },
  "stat": {
      "behavior": 32772,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castPoint": [
          0.0
      ],
      "cooldown": [
          45.0
      ],
      "manaCost": [
          125.0
      ],
      "sharedCooldown": "",
      "cost": 4700,
      "shopTags": "agi;str;int;attack_speed;move_speed;hard_to_tag",
      "aliases": "manta style",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 170
      },
      {
          "index": 2,
          "componentId": 24
      }
  ],
  "image": "manta_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"148": {
  "id": 148,
  "name": "item_recipe_lesser_crit",
  "shortName": "recipe_lesser_crit",
  "language": {
      "displayName": "Crystalys Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 500,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"149": {
  "id": 149,
  "name": "item_lesser_crit",
  "displayName": "Crystalys",
  "shortName": "lesser_crit",
  "language": {
      "displayName": "Crystalys",
      "lore": [
          "A blade forged from rare crystals, it seeks weak points in enemy armor."
      ],
      "description": [
          "<h1>Passive: Critical Strike</h1>Grants each attack a 20% chance to deal 175% damage."
      ],
      "notes": [
          "Critical Strike does not work against buildings."
      ],
      "attributes": [
          "+38 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2130,
      "shopTags": "damage;crit",
      "aliases": "crystalys",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 3
      },
      {
          "index": 2,
          "componentId": 2
      }
  ],
  "image": "lesser_crit_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"150": {
  "id": 150,
  "name": "item_recipe_armlet",
  "shortName": "recipe_armlet",
  "language": {
      "displayName": "Armlet of Mordiggian Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 650,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"151": {
  "id": 151,
  "name": "item_armlet",
  "displayName": "Armlet of Mordiggian",
  "shortName": "armlet",
  "language": {
      "displayName": "Armlet of Mordiggian",
      "lore": [
          "Weapon of choice among brutes, the bearer sacrifices his life energy to gain immense strength and power."
      ],
      "description": [
          "<h1>Toggle: Unholy Strength</h1>When active, Unholy Strength grants +31 damage, +25 strength and +4 armor, but drains 54 health per second.<br><br> You cannot die from the health drain when Unholy Strength is activated, nor from the strength loss when Unholy Strength is deactivated."
      ],
      "notes": [
          "The strength change will affect both maximum and current HP, but you cannot die from the change.",
          "The strength change occurs over 0.6 seconds.",
          "Activating or deactivating Unholy Strength does not interrupt channeling."
      ],
      "attributes": [
          "+9 Damage",
          "+25 Attack Speed",
          "+5 Armor",
          "+5 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 524804,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 2380,
      "shopTags": "damage;attack_speed;armor;regen_health;hard_to_tag",
      "aliases": "armlet of mordiggian",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 6
      },
      {
          "index": 2,
          "componentId": 25
      },
      {
          "index": 3,
          "componentId": 2
      }
  ],
  "image": "armlet_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"152": {
  "id": 152,
  "name": "item_invis_sword",
  "displayName": "Shadow Blade",
  "shortName": "invis_sword",
  "language": {
      "displayName": "Shadow Blade",
      "lore": [
          "The blade of a fallen king, it allows you to move unseen and strike from the shadows."
      ],
      "description": [
          "<h1>Active: Shadow Walk</h1>Makes you invisible for 14.0 seconds, or until you attack or cast a spell.  While Shadow Walk is active, you move 20 faster and can move through units.  <br><br>If attacking to end the invisibility, you gain 175 bonus physical damage on that attack."
      ],
      "notes": [
          "Has a %windwalk_fade_time% second fade time.",
          "If the invisibility ends without attacking, the bonus damage is lost."
      ],
      "attributes": [
          "+27 Damage",
          "+30 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 526340,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "cooldown": [
          28.0
      ],
      "manaCost": [
          75.0
      ],
      "sharedCooldown": "shadow_blade",
      "cost": 2800,
      "shopTags": "damage;attack_speed;movespeed;hard_to_tag",
      "aliases": "sb;invis;shadow blade;silver edge",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 215
      },
      {
          "index": 2,
          "componentId": 5
      }
  ],
  "image": "invis_sword_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"153": {
  "id": 153,
  "name": "item_recipe_sange_and_yasha",
  "shortName": "recipe_sange_and_yasha",
  "language": {
      "displayName": "Recipe: Sange and Yasha",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"154": {
  "id": 154,
  "name": "item_sange_and_yasha",
  "displayName": "Sange and Yasha",
  "shortName": "sange_and_yasha",
  "language": {
      "displayName": "Sange and Yasha",
      "lore": [
          "Sange and Yasha, when attuned by the moonlight and used together, become a very powerful combination."
      ],
      "description": [],
      "notes": [
          "Yasha-based movement speed bonuses from multiple items do not stack."
      ],
      "attributes": [
          "+16 Strength",
          "+16 Agility",
          "+16 Attack Speed",
          "+30 Movement Speed",
          "+20% Status Resistance",
          "+30% Self HP Regen and Lifesteal Amp"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 4100,
      "shopTags": "damage;str;agi;attack_speed;unique",
      "aliases": "sny;s&y;sy;sange and yasha",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 170
      },
      {
          "index": 2,
          "componentId": 162
      }
  ],
  "image": "sange_and_yasha_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"155": {
  "id": 155,
  "name": "item_recipe_satanic",
  "shortName": "recipe_satanic",
  "language": {
      "displayName": "Recipe: Satanic",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"156": {
  "id": 156,
  "name": "item_satanic",
  "displayName": "Satanic",
  "shortName": "satanic",
  "language": {
      "displayName": "Satanic",
      "lore": [
          "Immense power at the cost of your soul."
      ],
      "description": [
          "<h1>Active: Unholy Rage</h1>Increases Lifesteal percentage to 200% for 5.0 seconds. ",
          "<h1>Passive: Lifesteal</h1>Heals the attacker for 25% of attack damage dealt."
      ],
      "notes": [],
      "attributes": [
          "+25 Strength",
          "+25 Damage",
          "+30% Status Resistance"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 2,
      "cooldown": [
          35.0
      ],
      "sharedCooldown": "",
      "cost": 5300,
      "shopTags": "damage;str;armor;unique;hard_to_tag",
      "aliases": "satanic",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 26
      },
      {
          "index": 2,
          "componentId": 5
      },
      {
          "index": 3,
          "componentId": 53
      }
  ],
  "image": "satanic_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"157": {
  "id": 157,
  "name": "item_recipe_mjollnir",
  "shortName": "recipe_mjollnir",
  "language": {
      "displayName": "Mjollnir Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 900,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"158": {
  "id": 158,
  "name": "item_mjollnir",
  "displayName": "Mjollnir",
  "shortName": "mjollnir",
  "language": {
      "displayName": "Mjollnir",
      "lore": [
          "Thor's magical hammer, made for him by the dwarves Brok and Eitri."
      ],
      "description": [
          "<h1>Active: Static Charge</h1>Places a charged shield on a target unit for 15.0 seconds which has a 20% chance to release a 200 magical damage shocking bolt at a nearby attacker and 4 additional enemies.<br><br>Range: %abilitycastrange%",
          "<h1>Passive: Chain Lightning</h1>Grants a 25% chance on attack to release a bolt of electricity that leaps between 12 targets within a 900 radius, dealing 170 magical damage to each. Lightning proc pierces evasion."
      ],
      "notes": [
          "Static Charge cannot trigger more than once per second.",
          "Static Charge's shock deals magical damage centered on hero with the Static Charge. Static Charge's targets cannot be more than %static_seconary_radius% range away.",
          "Static Charge procs will not bounce to heroes that are invisible or hidden by Fog of War."
      ],
      "attributes": [
          "+24 Damage",
          "+75 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          800
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          35.0
      ],
      "manaCost": [
          50.0
      ],
      "sharedCooldown": "",
      "cost": 5600,
      "shopTags": "damage;attack_speed;unique",
      "aliases": "mjollnir",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 166
      },
      {
          "index": 2,
          "componentId": 55
      }
  ],
  "image": "mjollnir_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"159": {
  "id": 159,
  "name": "item_recipe_skadi",
  "shortName": "recipe_skadi",
  "language": {
      "displayName": "Recipe: Eye of Skadi",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"160": {
  "id": 160,
  "name": "item_skadi",
  "displayName": "Eye of Skadi",
  "shortName": "skadi",
  "language": {
      "displayName": "Eye of Skadi",
      "lore": [
          "Extremely rare artifact, guarded by the azure dragons."
      ],
      "description": [
          "<h1>Passive: Cold Attack</h1> Attacks lower enemy movement and attack speed for 3.0 seconds. Slows enemy ranged units by -45 movement speed and by -45 attack speed. Slows enemy melee units by -20% movement speed and by -20 attack speed. "
      ],
      "notes": [],
      "attributes": [
          "+25 All Attributes",
          "+225 Health",
          "+250 Mana"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 5500,
      "shopTags": "agi;str;int;hard_to_tag;mana_pool;health_pool;unique",
      "aliases": "eos;eye of skadi",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 24
      },
      {
          "index": 2,
          "componentId": 24
      },
      {
          "index": 3,
          "componentId": 60
      }
  ],
  "image": "skadi_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"161": {
  "id": 161,
  "name": "item_recipe_sange",
  "shortName": "recipe_sange",
  "language": {
      "displayName": "Sange Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 600,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"162": {
  "id": 162,
  "name": "item_sange",
  "displayName": "Sange",
  "shortName": "sange",
  "language": {
      "displayName": "Sange",
      "lore": [
          "Sange is an unusually accurate weapon, seeking weak points automatically."
      ],
      "description": [],
      "notes": [],
      "attributes": [
          "+16 Strength",
          "+16% Status Resistance",
          "+24% Self HP Regen and Lifesteal Amp"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2050,
      "shopTags": "damage;str;unique",
      "aliases": "sange",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 21
      },
      {
          "index": 2,
          "componentId": 17
      }
  ],
  "image": "sange_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"163": {
  "id": 163,
  "name": "item_recipe_helm_of_the_dominator",
  "shortName": "recipe_helm_of_the_dominator",
  "language": {
      "displayName": "Helm of the Dominator Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 300,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"164": {
  "id": 164,
  "name": "item_helm_of_the_dominator",
  "displayName": "Helm of the Dominator",
  "shortName": "helm_of_the_dominator",
  "language": {
      "displayName": "Helm of the Dominator",
      "lore": [
          "The powerful headpiece of a dead necromancer."
      ],
      "description": [
          "<h1>Active: Dominate</h1>Takes control of one neutral, non-ancient target unit and sets its movement speed to 425 and max health to a minimum of 1500.<br><br>Dominated units with a max health of greater than 1500 retain their original max health.  Dominated unit's bounty is set to 200 gold.<br><br>Range: %abilitycastrange%",
          "<h1>Passive: Dominator Aura</h1> Increases nearby allied hero and player units base damage by 20%  and health regen by 5. Has a 50  bigger effect on player units.<br><br>Radius: 1200"
      ],
      "notes": [
          "Cannot dominate more than one unit at a time.  If a new unit is dominated, the old one will die.",
          "Can also Dominate enemy lane creeps and summoned units.",
          "Selling Helm of the Dominator will cause dominated units to die."
      ],
      "attributes": [
          "+7 All Attributes"
      ]
  },
  "stat": {
      "behavior": 32776,
      "unitTargetType": 4,
      "unitTargetTeam": 1,
      "unitTargetFlags": 5,
      "fightLevelRecap": 0,
      "castRange": [
          700
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          60.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 2400,
      "shopTags": "damage;armor;unique;hard_to_tag",
      "aliases": "hotd;helm of the dominator",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 3
      },
      {
          "index": 2,
          "componentId": 94
      },
      {
          "index": 3,
          "componentId": 261
      }
  ],
  "image": "helm_of_the_dominator_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"165": {
  "id": 165,
  "name": "item_recipe_maelstrom",
  "shortName": "recipe_maelstrom",
  "language": {
      "displayName": "Maelstrom Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"166": {
  "id": 166,
  "name": "item_maelstrom",
  "displayName": "Maelstrom",
  "shortName": "maelstrom",
  "language": {
      "displayName": "Maelstrom",
      "lore": [
          "A hammer forged for the gods themselves, Maelstrom allows its user to harness the power of lightning."
      ],
      "description": [
          "<h1>Passive: Chain Lightning</h1>Grants a 25% chance on attack to release a bolt of electricity that leaps between 4 targets within a 900 radius, dealing 150 magical damage to each. Lightning proc pierces evasion."
      ],
      "notes": [],
      "attributes": [
          "+24 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2700,
      "shopTags": "damage;attack_speed;unique",
      "aliases": "maelstrom",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 8
      },
      {
          "index": 2,
          "componentId": 7
      }
  ],
  "image": "maelstrom_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"167": {
  "id": 167,
  "name": "item_recipe_desolator",
  "shortName": "recipe_desolator",
  "language": {
      "displayName": "Recipe: Desolator",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"168": {
  "id": 168,
  "name": "item_desolator",
  "displayName": "Desolator",
  "shortName": "desolator",
  "language": {
      "displayName": "Desolator",
      "lore": [
          "A wicked weapon, used in torturing political criminals."
      ],
      "description": [
          "<h1>Passive: Corruption</h1> Your attacks reduce the target's armor by -7 for 15.0 seconds."
      ],
      "notes": [
          "Armor reduction works on buildings."
      ],
      "attributes": [
          "+50 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 3500,
      "shopTags": "damage;unique",
      "aliases": "desolator",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 8
      },
      {
          "index": 2,
          "componentId": 8
      },
      {
          "index": 3,
          "componentId": 240
      }
  ],
  "image": "desolator_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"169": {
  "id": 169,
  "name": "item_recipe_yasha",
  "shortName": "recipe_yasha",
  "language": {
      "displayName": "Yasha Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 600,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"170": {
  "id": 170,
  "name": "item_yasha",
  "displayName": "Yasha",
  "shortName": "yasha",
  "language": {
      "displayName": "Yasha",
      "lore": [
          "Yasha is regarded as the swiftest weapon ever created."
      ],
      "description": [],
      "notes": [
          "Yasha-based movement speed bonuses from multiple items do not stack."
      ],
      "attributes": [
          "+16 Agility",
          "+12 Attack Speed",
          "+20 Movement Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2050,
      "shopTags": "agi;attack_speed;move_speed",
      "aliases": "yasha",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 22
      },
      {
          "index": 2,
          "componentId": 18
      }
  ],
  "image": "yasha_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"171": {
  "id": 171,
  "name": "item_recipe_mask_of_madness",
  "shortName": "recipe_mask_of_madness",
  "language": {
      "displayName": "Mask of Madness Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"172": {
  "id": 172,
  "name": "item_mask_of_madness",
  "displayName": "Mask of Madness",
  "shortName": "mask_of_madness",
  "language": {
      "displayName": "Mask of Madness",
      "lore": [
          "Once this mask is worn, its bearer becomes an uncontrollable aggressive force."
      ],
      "description": [
          "<h1>Active: Berserk</h1>Gives 110 attack speed and 30 movement speed, but reduces your armor by 8 and silences you.  Lasts 6.0 seconds.",
          "<h1>Passive: Lifesteal</h1>Heals the attacker for 20% of attack damage dealt."
      ],
      "notes": [],
      "attributes": [
          "+10 Damage",
          "+10 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 526340,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "cooldown": [
          16.0
      ],
      "manaCost": [
          25.0
      ],
      "sharedCooldown": "",
      "cost": 1775,
      "shopTags": "unique;hard_to_tag",
      "aliases": "mom;mask of madness",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 26
      },
      {
          "index": 2,
          "componentId": 10
      }
  ],
  "image": "mask_of_madness_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"173": {
  "id": 173,
  "name": "item_recipe_diffusal_blade",
  "shortName": "recipe_diffusal_blade",
  "language": {
      "displayName": "Diffusal Blade Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 700,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"174": {
  "id": 174,
  "name": "item_diffusal_blade",
  "displayName": "Diffusal Blade",
  "shortName": "diffusal_blade",
  "language": {
      "displayName": "Diffusal Blade",
      "lore": [
          "An enchanted blade that allows the user to cut straight into the enemy's soul."
      ],
      "description": [
          "<h1>Active: Inhibit</h1> Targets an enemy, slowing it for 4.0 seconds.<br><br>Range: %abilitycastrange%",
          "<h1>Passive: Manabreak</h1>Each attack burns 50 mana from the target, and deals 0.8 physical damage per burned mana. <br><br>Burns 16 mana per attack from melee illusions and 8 mana per attack from ranged illusions."
      ],
      "notes": [
          "Does not stack with other manabreak abilities."
      ],
      "attributes": [
          "+20 Agility",
          "+10 Intelligence"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          600
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          15.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "diffusal",
      "cost": 3150,
      "shopTags": "agi;int;unique;hard_to_tag",
      "aliases": "diffusal blade 1",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": true,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 22
      },
      {
          "index": 2,
          "componentId": 22
      },
      {
          "index": 3,
          "componentId": 19
      }
  ],
  "image": "diffusal_blade_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"175": {
  "id": 175,
  "name": "item_recipe_ethereal_blade",
  "shortName": "recipe_ethereal_blade",
  "language": {
      "displayName": "Recipe: Ethereal Blade",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"176": {
  "id": 176,
  "name": "item_ethereal_blade",
  "displayName": "Ethereal Blade",
  "shortName": "ethereal_blade",
  "language": {
      "displayName": "Ethereal Blade",
      "lore": [
          "A flickering blade of a ghastly nature, it is capable of dealing damage in both magical and physical planes."
      ],
      "description": [
          "<h1>Active: Ether Blast</h1>Converts the target unit to ethereal form, rendering them immune to physical damage, but unable to attack and -40 more vulnerable to magic damage.  Lasts for 4.0 seconds on allies and 4.0 seconds on enemies.<br><br>  Enemy targets are also slowed by -80, and take 2.0x your primary attribute + 75 as magical damage.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [
          "Shares cooldown with Ghost Scepter.",
          "Ethereal units take 40% bonus magic damage.",
          "Lasts an extra second on Self or Allied cast.",
          "Using a Town Portal Scroll or Boots of Travel will not dispel Ethereal Form."
      ],
      "attributes": [
          "+40 Agility",
          "+10 Strength",
          "+10 Intelligence"
      ]
  },
  "stat": {
      "behavior": 32776,
      "unitTargetType": 5,
      "unitTargetTeam": 3,
      "unitTargetFlags": 2,
      "fightLevelRecap": 1,
      "castRange": [
          800
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          20.0
      ],
      "manaCost": [
          100.0
      ],
      "sharedCooldown": "ethereal",
      "cost": 4700,
      "shopTags": "agi;str;int;hard_to_tag",
      "aliases": "eb;ethereal blade;eblade",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 52
      },
      {
          "index": 2,
          "componentId": 37
      }
  ],
  "image": "ethereal_blade_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"177": {
  "id": 177,
  "name": "item_recipe_soul_ring",
  "shortName": "recipe_soul_ring",
  "language": {
      "displayName": "Soul Ring Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 250,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"178": {
  "id": 178,
  "name": "item_soul_ring",
  "displayName": "Soul Ring",
  "shortName": "soul_ring",
  "language": {
      "displayName": "Soul Ring",
      "lore": [
          "A ring that feeds on the souls of those who wear it."
      ],
      "description": [
          "<h1>Active: Sacrifice</h1> Consume 170 health to temporarily gain 150 mana.  Lasts 10 seconds.<br><br>If the mana gained cannot fit in your mana pool, it creates a buffer of mana that will be used before your mana pool."
      ],
      "notes": [
          "If this mana is not used before the duration ends, the extra mana is lost."
      ],
      "attributes": [
          "+6 Strength",
          "+2.5 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          25.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 790,
      "shopTags": "regen_health;boost_mana",
      "aliases": "soul ring",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 27
      },
      {
          "index": 2,
          "componentId": 13
      },
      {
          "index": 3,
          "componentId": 13
      }
  ],
  "image": "soul_ring_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"179": {
  "id": 179,
  "name": "item_recipe_arcane_boots",
  "shortName": "recipe_arcane_boots",
  "language": {
      "displayName": "Recipe: Arcane Boots",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"180": {
  "id": 180,
  "name": "item_arcane_boots",
  "displayName": "Arcane Boots",
  "shortName": "arcane_boots",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Arcane Boots",
      "lore": [
          "Magi equipped with these boots are valued in battle."
      ],
      "description": [
          "<h1>Active: Replenish Mana</h1>Restores 160 mana to all nearby allies.<br><br>Radius: 1200",
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [
          "Does not work on Meepo clones."
      ],
      "attributes": [
          "+15% Movement Speed",
          "+250 Mana"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "cooldown": [
          55.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 1400,
      "shopTags": "move_speed;boost_mana;mana_pool",
      "aliases": "mana;mb;arcane boots",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isAlertable": true,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 29
      },
      {
          "index": 2,
          "componentId": 59
      }
  ],
  "image": "arcane_boots_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": false
},
"181": {
  "id": 181,
  "name": "item_orb_of_venom",
  "displayName": "Orb of Venom",
  "shortName": "orb_of_venom",
  "language": {
      "displayName": "Orb of Venom",
      "lore": [
          "Envenoms your veapon with the venom of a venomous viper."
      ],
      "description": [
          "<h1>Passive: Poison Attack</h1>Poisons the target, dealing 5.0 magical damage per second and slowing movement by -12 if the equipped hero is melee, or 3.0 magical damage per second and slowing movement by -4 if they are ranged. Lasts for 3.0 seconds."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 300,
      "shopTags": "hard_to_tag",
      "aliases": "oov;orb of venom",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "orb_of_venom_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"182": {
  "id": 182,
  "name": "item_stout_shield",
  "displayName": "Stout Shield",
  "shortName": "stout_shield",
  "language": {
      "displayName": "Stout Shield",
      "lore": [
          "One man's wine barrel bottom is another man's shield."
      ],
      "description": [
          "<h1>Passive: Damage Block</h1>Grants a 50% chance to block 20 damage from incoming attacks on melee heroes, and 8 damage on ranged."
      ],
      "notes": [
          "Multiple sources of damage block do not stack."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 100,
      "shopTags": "block",
      "aliases": "stout shield",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "stout_shield_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"183": {
  "id": 183,
  "name": "item_recipe_invis_sword",
  "shortName": "recipe_invis_sword",
  "language": {
      "displayName": "Recipe: Shadow Blade",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"184": {
  "id": 184,
  "name": "item_recipe_ancient_janggo",
  "shortName": "recipe_ancient_janggo",
  "language": {
      "displayName": "Drum of Endurance Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 800,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"185": {
  "id": 185,
  "name": "item_ancient_janggo",
  "displayName": "Drum of Endurance",
  "shortName": "ancient_janggo",
  "language": {
      "displayName": "Drum of Endurance",
      "lore": [
          "A relic that enchants the bodies of those around it for swifter movement in times of crisis."
      ],
      "description": [
          "<h1>Active: Endurance</h1> Gives +35 attack speed and +13% movement speed to nearby allies for 6 seconds. <br><br>Radius: 1200 ",
          "",
          "<h1>Passive: Speed Aura</h1> Increases nearby allies' attack speed by 20.<br><br>Radius: 1200"
      ],
      "notes": [
          "Re-purchasing the Drum of Endurance recipe will refresh its charges.",
          "Multiple instances of Swiftness Aura do not stack."
      ],
      "attributes": [
          "+4 Intelligence",
          "+4 Strength",
          "+4 Agility",
          "+20 Movement Speed"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          1200
      ],
      "cooldown": [
          30.0
      ],
      "sharedCooldown": "",
      "cost": 1900,
      "shopTags": "str;agi;int;damage;move_speed;attack_speed;hard_to_tag",
      "aliases": "drum of endurance",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": true,
      "isHideCharges": false,
      "isRequiresCharges": true,
      "isDisplayCharges": true,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 6,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 25
      },
      {
          "index": 2,
          "componentId": 261
      },
      {
          "index": 3,
          "componentId": 244
      }
  ],
  "image": "ancient_janggo_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"186": {
  "id": 186,
  "name": "item_recipe_medallion_of_courage",
  "shortName": "recipe_medallion_of_courage",
  "language": {
      "displayName": "Recipe: Medallion of Courage",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"187": {
  "id": 187,
  "name": "item_medallion_of_courage",
  "displayName": "Medallion of Courage",
  "shortName": "medallion_of_courage",
  "language": {
      "displayName": "Medallion of Courage",
      "lore": [
          "The bearer has no fear of combat."
      ],
      "description": [
          "<h1>Active: Valor</h1> If cast on an ally, grants them the 5 armor, and removing it from the caster. <br><br>If cast on an enemy, reduces 5 armor on both the enemy and the caster. <br><br>Cannot target spell immune enemies.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+5 Armor",
          "+0.75 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 4,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          1000
      ],
      "cooldown": [
          7.0
      ],
      "sharedCooldown": "medallion",
      "cost": 1100,
      "shopTags": "armor;regen_mana;hard_to_tag",
      "aliases": "medallion of courage",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 4
      },
      {
          "index": 2,
          "componentId": 28
      },
      {
          "index": 3,
          "componentId": 240
      }
  ],
  "image": "medallion_of_courage_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"188": {
  "id": 188,
  "name": "item_smoke_of_deceit",
  "displayName": "Smoke of Deceit",
  "shortName": "smoke_of_deceit",
  "language": {
      "displayName": "Smoke of Deceit",
      "lore": [
          "The charlatan wizard Myrddin's only true contribution to the arcane arts."
      ],
      "description": [
          "<h1>Use: Disguise</h1> Turns the caster and all allied player-controlled units in a 1200 radius invisible, and grants 15% bonus movement speed for 35.0 seconds. <br><br>Attacking, or moving within 1025 range of an enemy hero or tower, will break the invisibility. <br><br>Disguise grants invisibility that is immune to True Sight."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 32772,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          1200
      ],
      "cooldown": [
          1.0
      ],
      "sharedCooldown": "",
      "cost": 80,
      "shopTags": "consumable",
      "aliases": "smoke of deceit",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": true,
      "isSupport": true,
      "isAlertable": true,
      "isTempestDoubleClonable": false,
      "stockMax": 2,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 420,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "smoke_of_deceit_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"189": {
  "id": 189,
  "name": "item_recipe_veil_of_discord",
  "shortName": "recipe_veil_of_discord",
  "language": {
      "displayName": "Veil of Discord Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1150,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"190": {
  "id": 190,
  "name": "item_veil_of_discord",
  "displayName": "Veil of Discord",
  "shortName": "veil_of_discord",
  "language": {
      "displayName": "Veil of Discord",
      "lore": [
          "The headwear of corrupt magi."
      ],
      "description": [
          "<h1>Active: Magic Weakness</h1> Cast a 600 radius blast that causes enemies to take 20 increased damage from spells.<br><br>Range: %abilitycastrange%<br>Duration: 16.0 seconds. ",
          "",
          " <h1>Passive: Basilius Aura</h1>Grants 1.25 mana regeneration to allies.  <br><br>Radius: 1200"
      ],
      "notes": [],
      "attributes": [
          "+9 All Attributes"
      ]
  },
  "stat": {
      "behavior": 134217776,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1000
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          20.0
      ],
      "manaCost": [
          50.0
      ],
      "sharedCooldown": "",
      "cost": 2050,
      "shopTags": "int;armor;regen_health;hard_to_tag",
      "aliases": "vod;veil of discord",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 88
      },
      {
          "index": 2,
          "componentId": 261
      }
  ],
  "image": "veil_of_discord_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"191": {
  "id": 191,
  "name": "item_recipe_necronomicon_2",
  "shortName": "recipe_necronomicon_2",
  "language": {
      "displayName": "Recipe: Necronomicon (level 2)",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"192": {
  "id": 192,
  "name": "item_recipe_necronomicon_3",
  "shortName": "recipe_necronomicon_3",
  "language": {
      "displayName": "Recipe: Necronomicon (level 3)",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"193": {
  "id": 193,
  "name": "item_necronomicon_2",
  "displayName": "Necronomicon",
  "shortName": "necronomicon_2",
  "language": {
      "displayName": "Necronomicon (level 2)",
      "lore": [
          "Considered the ultimate in necromancy and demonology, a powerful malefic force is locked within its pages."
      ],
      "description": [
          "<h1>Active: Demonic Summoning</h1>Summons a Warrior and an Archer to fight for you for 60 seconds.<br><br><h1>Warrior:</h1>Burns mana every hit, and deals magical damage to whoever kills it.  Gains True Sight at level 3.<br>Health: 700 / 800 / 900<br>Damage: 75 / 100 / 125<br>Mana Break Damage: 30 / 40 / 50<br>Last Will Damage: 600 / 700 / 800<br><br><h1>Archer:</h1>Has a passive movement and attack speed aura. Gains Purge at Level 3.<br>Health: 700 / 800 / 900<br>Damage: 60 / 90 / 120<br>Aura Move Speed: 5 / 7 / 9<br>Aura Attack Speed: 5 / 10 / 15<br>Aura Radius: 1200"
      ],
      "notes": [],
      "attributes": [
          "+10 / 15 / 20 Strength",
          "+2 / 2.5 / 3.0 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castPoint": [
          0.0
      ],
      "cooldown": [
          90.0
      ],
      "manaCost": [
          50.0
      ],
      "sharedCooldown": "necronomicon",
      "cost": 3550,
      "shopTags": "int;str;hard_to_tag;see_invis",
      "aliases": "necronomicon 2",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 106
      },
      {
          "index": 2,
          "componentId": 105
      }
  ],
  "image": "necronomicon_2_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"194": {
  "id": 194,
  "name": "item_necronomicon_3",
  "displayName": "Necronomicon",
  "shortName": "necronomicon_3",
  "language": {
      "displayName": "Necronomicon (level 3)",
      "lore": [
          "Considered the ultimate in necromancy and demonology, a powerful malefic force is locked within its pages."
      ],
      "description": [
          "<h1>Active: Demonic Summoning</h1>Summons a Warrior and an Archer to fight for you for 60 seconds.<br><br><h1>Warrior:</h1>Burns mana every hit, and deals magical damage to whoever kills it.  Gains True Sight at level 3.<br>Health: 700 / 800 / 900<br>Damage: 75 / 100 / 125<br>Mana Break Damage: 30 / 40 / 50<br>Last Will Damage: 600 / 700 / 800<br><br><h1>Archer:</h1>Has a passive movement and attack speed aura. Gains Purge at Level 3.<br>Health: 700 / 800 / 900<br>Damage: 60 / 90 / 120<br>Aura Move Speed: 5 / 7 / 9<br>Aura Attack Speed: 5 / 10 / 15<br>Aura Radius: 1200"
      ],
      "notes": [],
      "attributes": [
          "+10 / 15 / 20 Strength",
          "+2 / 2.5 / 3.0 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castPoint": [
          0.0
      ],
      "cooldown": [
          90.0
      ],
      "manaCost": [
          50.0
      ],
      "sharedCooldown": "necronomicon",
      "cost": 4850,
      "shopTags": "int;str;hard_to_tag;see_invis",
      "aliases": "necronomicon 3",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 193
      },
      {
          "index": 2,
          "componentId": 105
      }
  ],
  "image": "necronomicon_3_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"195": {
  "id": 195,
  "name": "item_recipe_diffusal_blade_2",
  "displayName": "Recipe: Diffusal Blade (level 2)",
  "shortName": "recipe_diffusal_blade_2",
  "language": {
      "displayName": "Recipe: Diffusal Blade (level 2)",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"196": {
  "id": 196,
  "name": "item_diffusal_blade_2",
  "displayName": "Diffusal Blade (level 2)",
  "shortName": "diffusal_blade_2",
  "language": {
      "displayName": "Diffusal Blade (level 2)",
      "lore": [
          "An enchanted blade that allows the user to cut straight into the enemy's soul."
      ],
      "description": [
          "<h1>Active: Purge</h1> Targets an enemy, removing buffs from the target and slowing it for %purge_slow_duration% seconds.<br><br>Range: %abilitycastrange%",
          "<h1>Passive: Manabreak</h1>Each attack burns %feedback_mana_burn% mana from the target, and deals %damage_per_burn% physical damage per burned mana. <br><br>Burns %feedback_mana_burn_illusion_melee% mana per attack from melee illusions and %feedback_mana_burn_illusion_ranged% mana per attack from ranged illusions. <br><br>Dispel Type: <span class=\\\"GameplayValues GameplayVariable\\\">Basic Dispel</span>"
      ],
      "notes": [
          "Does not stack with other manabreak abilities."
      ],
      "attributes": []
  },
  "image": "diffusal_blade_2_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"197": {
  "id": 197,
  "name": "item_recipe_dagon_2",
  "shortName": "recipe_dagon_2",
  "language": {
      "displayName": "Recipe: Dagon (level 2)",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"198": {
  "id": 198,
  "name": "item_recipe_dagon_3",
  "shortName": "recipe_dagon_3",
  "language": {
      "displayName": "Recipe: Dagon (level 3)",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"199": {
  "id": 199,
  "name": "item_recipe_dagon_4",
  "shortName": "recipe_dagon_4",
  "language": {
      "displayName": "Recipe: Dagon (level 4)",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"200": {
  "id": 200,
  "name": "item_recipe_dagon_5",
  "shortName": "recipe_dagon_5",
  "language": {
      "displayName": "Recipe: Dagon (level 5)",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"201": {
  "id": 201,
  "name": "item_dagon_2",
  "displayName": "Dagon",
  "shortName": "dagon_2",
  "language": {
      "displayName": "Dagon (level 2)",
      "lore": [
          "A lesser wand that grows in power the longer it is used, it brings magic to the fingertips of the user."
      ],
      "description": [
          "<h1>Active: Energy Burst</h1> Emits a powerful burst of magical damage upon a targeted enemy unit. Upgradable.<br><br>Damage: 400 / 500 / 600 / 700 / 800<br>Range: %abilitycastrange%<br>Mana Cost: 120 / 140 / 160 / 180 / 200"
      ],
      "notes": [
          "Instantly kills illusions."
      ],
      "attributes": [
          "+13 / 16 / 19 / 22 / 25 Intelligence",
          "+5 All Attributes"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          600,
          650,
          700,
          750,
          800
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          35.0,
          30.0,
          25.0,
          20.0,
          15.0
      ],
      "manaCost": [
          120.0,
          140.0,
          160.0,
          180.0,
          200.0
      ],
      "sharedCooldown": "dagon",
      "cost": 3950,
      "shopTags": "damage;int;str;agi;hard_to_tag",
      "aliases": "dagon 2",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 104
      },
      {
          "index": 2,
          "componentId": 103
      }
  ],
  "image": "dagon_2_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"202": {
  "id": 202,
  "name": "item_dagon_3",
  "displayName": "Dagon",
  "shortName": "dagon_3",
  "language": {
      "displayName": "Dagon (level 3)",
      "lore": [
          "A lesser wand that grows in power the longer it is used, it brings magic to the fingertips of the user."
      ],
      "description": [
          "<h1>Active: Energy Burst</h1> Emits a powerful burst of magical damage upon a targeted enemy unit. Upgradable.<br><br>Damage: 400 / 500 / 600 / 700 / 800<br>Range: %abilitycastrange%<br>Mana Cost: 120 / 140 / 160 / 180 / 200"
      ],
      "notes": [
          "Instantly kills illusions."
      ],
      "attributes": [
          "+13 / 16 / 19 / 22 / 25 Intelligence",
          "+5 All Attributes"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          600,
          650,
          700,
          750,
          800
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          35.0,
          30.0,
          25.0,
          20.0,
          15.0
      ],
      "manaCost": [
          120.0,
          140.0,
          160.0,
          180.0,
          200.0
      ],
      "sharedCooldown": "dagon",
      "cost": 5200,
      "shopTags": "damage;int;str;agi;hard_to_tag",
      "aliases": "dagon 3",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 201
      },
      {
          "index": 2,
          "componentId": 103
      }
  ],
  "image": "dagon_3_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"203": {
  "id": 203,
  "name": "item_dagon_4",
  "displayName": "Dagon",
  "shortName": "dagon_4",
  "language": {
      "displayName": "Dagon (level 4)",
      "lore": [
          "A lesser wand that grows in power the longer it is used, it brings magic to the fingertips of the user."
      ],
      "description": [
          "<h1>Active: Energy Burst</h1> Emits a powerful burst of magical damage upon a targeted enemy unit. Upgradable.<br><br>Damage: 400 / 500 / 600 / 700 / 800<br>Range: %abilitycastrange%<br>Mana Cost: 120 / 140 / 160 / 180 / 200"
      ],
      "notes": [
          "Instantly kills illusions."
      ],
      "attributes": [
          "+13 / 16 / 19 / 22 / 25 Intelligence",
          "+5 All Attributes"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          600,
          650,
          700,
          750,
          800
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          35.0,
          30.0,
          25.0,
          20.0,
          15.0
      ],
      "manaCost": [
          120.0,
          140.0,
          160.0,
          180.0,
          200.0
      ],
      "sharedCooldown": "dagon",
      "cost": 6450,
      "shopTags": "damage;int;str;agi;hard_to_tag",
      "aliases": "dagon 4",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 202
      },
      {
          "index": 2,
          "componentId": 103
      }
  ],
  "image": "dagon_4_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"204": {
  "id": 204,
  "name": "item_dagon_5",
  "displayName": "Dagon",
  "shortName": "dagon_5",
  "language": {
      "displayName": "Dagon (level 5)",
      "lore": [
          "A lesser wand that grows in power the longer it is used, it brings magic to the fingertips of the user."
      ],
      "description": [
          "<h1>Active: Energy Burst</h1> Emits a powerful burst of magical damage upon a targeted enemy unit.<br><br>Damage: 400 / 500 / 600 / 700 / 800<br>Range: %abilitycastrange%<br>Mana Cost: 120 / 140 / 160 / 180 / 200"
      ],
      "notes": [
          "Instantly kills illusions."
      ],
      "attributes": [
          "+13 / 16 / 19 / 22 / 25 Intelligence",
          "+5 All Attributes"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          600,
          650,
          700,
          750,
          800
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          35.0,
          30.0,
          25.0,
          20.0,
          15.0
      ],
      "manaCost": [
          120.0,
          140.0,
          160.0,
          180.0,
          200.0
      ],
      "sharedCooldown": "dagon",
      "cost": 7700,
      "shopTags": "damage;int;str;agi;hard_to_tag",
      "aliases": "dagon 5",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 203
      },
      {
          "index": 2,
          "componentId": 103
      }
  ],
  "image": "dagon_5_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"205": {
  "id": 205,
  "name": "item_recipe_rod_of_atos",
  "shortName": "recipe_rod_of_atos",
  "language": {
      "displayName": "Rod of Atos Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 850,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"206": {
  "id": 206,
  "name": "item_rod_of_atos",
  "displayName": "Rod of Atos",
  "shortName": "rod_of_atos",
  "language": {
      "displayName": "Rod of Atos",
      "lore": [
          "Atos, the Lord of Blight, has his essence stored in this deceptively simple wand."
      ],
      "description": [
          "<h1>Active: Cripple</h1>Roots the target for 2.0 seconds.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+20 Intelligence",
          "+10 Strength",
          "+10 Agility"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          1100
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          18.0
      ],
      "manaCost": [
          50.0
      ],
      "sharedCooldown": "",
      "cost": 2750,
      "shopTags": "int;regen_health",
      "aliases": "rod of atos",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 23
      },
      {
          "index": 2,
          "componentId": 261
      },
      {
          "index": 3,
          "componentId": 261
      }
  ],
  "image": "rod_of_atos_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"207": {
  "id": 207,
  "name": "item_recipe_abyssal_blade",
  "shortName": "recipe_abyssal_blade",
  "language": {
      "displayName": "Abyssal Blade Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1550,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"208": {
  "id": 208,
  "name": "item_abyssal_blade",
  "displayName": "Abyssal Blade",
  "shortName": "abyssal_blade",
  "language": {
      "displayName": "Abyssal Blade",
      "lore": [
          "The lost blade of the Commander of the Abyss, this edge cuts into an enemy's soul."
      ],
      "description": [
          "<h1>Active: Overwhelm</h1> Blinks to and stuns a target enemy unit for 2.0 seconds. <br><br>Pierces Spell Immunity.<br><br>Range: %abilitycastrange%",
          "<h1>Passive: Bash</h1> Grants melee heroes a 25% chance on hit to stun the target for 1.5 seconds and deal 100 bonus physical damage.  Bash chance for ranged heroes is 10%.",
          "<h1>Passive: Damage Block</h1> Grants a 50% chance to block 70 damage from incoming attacks on melee heroes, and 35 damage on ranged."
      ],
      "notes": [
          "The stun is melee range.",
          "Does not stack with other bashes.",
          "Multiple sources of damage block do not stack."
      ],
      "attributes": [
          "+25 Damage",
          "+250 Health",
          "+10 HP Regeneration",
          "+10 Strength"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 4,
      "fightLevelRecap": 1,
      "castRange": [
          600
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          35.0
      ],
      "manaCost": [
          75.0
      ],
      "sharedCooldown": "",
      "cost": 6650,
      "shopTags": "damage;str;hard_to_tag",
      "aliases": "abyssal blade",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 143
      },
      {
          "index": 2,
          "componentId": 125
      }
  ],
  "image": "abyssal_blade_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"209": {
  "id": 209,
  "name": "item_recipe_heavens_halberd",
  "shortName": "recipe_heavens_halberd",
  "language": {
      "displayName": "Recipe: Heaven's Halberd",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"210": {
  "id": 210,
  "name": "item_heavens_halberd",
  "displayName": "Heaven's Halberd",
  "shortName": "heavens_halberd",
  "language": {
      "displayName": "Heaven's Halberd",
      "lore": [
          "This halberd moves with the speed of a smaller weapon, allowing the bearer to win duels that a heavy edge would not."
      ],
      "description": [
          "<h1>Active: Disarm</h1>Prevents a target from attacking for 3.0 seconds on melee targets, and 5.0 seconds on ranged targets.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+25% Evasion",
          "+20 Strength",
          "+20% Status Resistance",
          "+30% Self HP Regen and Lifesteal Amp"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          600
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          18.0
      ],
      "manaCost": [
          100.0
      ],
      "sharedCooldown": "",
      "cost": 3450,
      "shopTags": "str;damage;evasion",
      "aliases": "heaven's halberd",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 162
      },
      {
          "index": 2,
          "componentId": 32
      }
  ],
  "image": "heavens_halberd_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"211": {
  "id": 211,
  "name": "item_recipe_ring_of_aquila",
  "shortName": "recipe_ring_of_aquila",
  "language": {
      "displayName": "Recipe: Ring of Aquila",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"212": {
  "id": 212,
  "name": "item_ring_of_aquila",
  "displayName": "Ring of Aquila",
  "shortName": "ring_of_aquila",
  "language": {
      "displayName": "Ring of Aquila",
      "lore": [
          "The ring of the fallen Warlord Aquila continues to support armies in battle."
      ],
      "description": [
          "<h1>Passive: Aquila Aura</h1>Grants 1.25 mana regeneration and 2 armor to nearby allies.<br><br>Radius: 1200",
          "<h1>Toggle: Aura</h1> Deactivate to stop affecting non-hero units."
      ],
      "notes": [
          "Does not stack with armor auras from Ring of Basilius or Ring of Aquila.",
          "Multiple instances of Aquila Aura do not stack."
      ],
      "attributes": [
          "+3 Strength",
          "+9 Agility",
          "+3 Intelligence",
          "+7 Damage"
      ]
  },
  "stat": {
      "behavior": 2564,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "aliases": "roa;ring of aquila",
      "quality": "rare",
      "isSellable": false,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ring_of_aquila_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"213": {
  "id": 213,
  "name": "item_recipe_tranquil_boots",
  "shortName": "recipe_tranquil_boots",
  "language": {
      "displayName": "Tranquil Boots Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"214": {
  "id": 214,
  "name": "item_tranquil_boots",
  "displayName": "Tranquil Boots",
  "shortName": "tranquil_boots",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Tranquil Boots",
      "lore": [
          "While they increase the longevity of the wearer, this boot is not particularly reliable."
      ],
      "description": [
          "<h1>Passive: Break</h1> Whenever you attack a hero or are attacked by any unit, the bonus 16 HP regen is lost and the movement speed bonus is reduced to 18 for 13 seconds.",
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [],
      "attributes": [
          "+26% Movement Speed",
          "+16 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          13.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 1000,
      "shopTags": "move_speed;regen_health;armor",
      "aliases": "tranquil boots",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": true,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 29
      },
      {
          "index": 2,
          "componentId": 244
      },
      {
          "index": 3,
          "componentId": 27
      }
  ],
  "image": "tranquil_boots_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"215": {
  "id": 215,
  "name": "item_shadow_amulet",
  "displayName": "Shadow Amulet",
  "shortName": "shadow_amulet",
  "language": {
      "displayName": "Shadow Amulet",
      "lore": [
          "A small talisman that clouds the senses of one's enemies when held perfectly still."
      ],
      "description": [
          "<h1>Active: Fade</h1>Grants invisibility to you or a target allied hero as long as the target unit remains still.  <br><br>Has a 1.25 second fade time, and breaks instantly upon moving. <br><br>There is no cooldown when using this item on yourself.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+20 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 534536,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          600
      ],
      "cooldown": [
          7.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 1400,
      "shopTags": "",
      "aliases": "shadow amulet",
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "shadow_amulet_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": false
},
"216": {
  "id": 216,
  "name": "item_enchanted_mango",
  "displayName": "Enchanted Mango",
  "shortName": "enchanted_mango",
  "language": {
      "displayName": "Enchanted Mango",
      "lore": [
          "The bittersweet flavors of Jidi Isle are irresistible to amphibians."
      ],
      "description": [
          "<h1>Use: Eat Mango</h1> Instantly restores 125 mana.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [
          "Hold Control to use on a nearby allied hero."
      ],
      "attributes": [
          "+0.6 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          400
      ],
      "castPoint": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 70,
      "shopTags": "consumable",
      "aliases": "enchanted mango",
      "quality": "consumable",
      "isSellable": false,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "enchanted_mango_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"217": {
  "id": 217,
  "name": "item_recipe_ward_dispenser",
  "shortName": "recipe_ward_dispenser",
  "language": {
      "displayName": "Recipe: Observer and Sentry Wards",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"218": {
  "id": 218,
  "name": "item_ward_dispenser",
  "displayName": "Observer and Sentry Wards",
  "shortName": "ward_dispenser",
  "language": {
      "displayName": "Observer and Sentry Wards",
      "lore": [
          "Advancements in stacking efficiency have made wards easier to carry than ever."
      ],
      "description": [
          "<h1>Use: Plant</h1> Plant the currently active ward.  Double-Click to switch the currently active ward.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [
          "Hold Control to give one ward to an allied hero."
      ],
      "attributes": [
          "600% OBSERVER VISION RANGE:",
          "% OBSERVER DURATION (MINUTES):",
          "000% SENTRY TRUE SIGHT RANGE:",
          "% SENTRY DURATION (MINUTES):"
      ]
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 16,
      "fightLevelRecap": 0,
      "castRange": [
          500
      ],
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 75,
      "aliases": "ward",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 43
      },
      {
          "index": 2,
          "componentId": 42
      }
  ],
  "image": "ward_dispenser_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"219": {
  "id": 219,
  "name": "item_recipe_travel_boots_2",
  "shortName": "recipe_travel_boots_2",
  "language": {
      "displayName": "Recipe: Boots of Travel",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"220": {
  "id": 220,
  "name": "item_travel_boots_2",
  "displayName": "Boots of Travel",
  "shortName": "travel_boots_2",
  "language": {
      "displayName": "Boots of Travel",
      "lore": [
          "Winged boots that grant omnipresence."
      ],
      "description": [
          "<h1>Active: Teleport</h1> Teleports you to any allied structure or unit, including heroes. Teleporting to a unit is interrupted if the target unit dies.<br><br>Adds 10 seconds to the cooldown of Town Portal Scroll.",
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [],
      "attributes": [
          "+35% Movement Speed"
      ]
  },
  "stat": {
      "behavior": 37020,
      "unitTargetType": 5,
      "unitTargetTeam": 2,
      "unitTargetFlags": 32,
      "fightLevelRecap": 0,
      "cooldown": [
          45.0
      ],
      "manaCost": [
          75.0
      ],
      "channelTime": [
          3.0
      ],
      "sharedCooldown": "travel",
      "cost": 4500,
      "shopTags": "teleport;move_speed",
      "aliases": "bot;boots of travel;tp",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 48
      },
      {
          "index": 2,
          "componentId": 47
      }
  ],
  "image": "travel_boots_2_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": false
},
"221": {
  "id": 221,
  "name": "item_recipe_lotus_orb",
  "shortName": "recipe_lotus_orb",
  "language": {
      "displayName": "Recipe: Lotus Orb",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"222": {
  "id": 222,
  "name": "item_recipe_meteor_hammer",
  "shortName": "recipe_meteor_hammer",
  "language": {
      "displayName": "Recipe: Meteor Hammer",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"223": {
  "id": 223,
  "name": "item_meteor_hammer",
  "displayName": "Meteor Hammer",
  "shortName": "meteor_hammer",
  "language": {
      "displayName": "Meteor Hammer",
      "lore": [
          "An enchanted hammer forged of metals discovered in the ruins of a village destroyed by a great star storm."
      ],
      "description": [
          "<h1>Active: Meteor Hammer</h1> CHANNELED - After a successful channel, summons a meteor that strikes a 315 AoE, stunning enemies for 2.0 seconds and dealing impact damage. Continues to deal damage over time to enemies units and buildings for 6 seconds.<BR><BR>Building Impact Damage: 75 <BR>Building Over Time Damage: 50 <BR><BR>Non-Building Impact Damage: 150 <BR>Non-Building Over Time Damage: 90 <BR><BR>Channel Duration: 2.5 seconds.<BR>Landing Time: .5 seconds."
      ],
      "notes": [],
      "attributes": [
          "+12 Strength",
          "+12 Intelligence",
          "+5.0 HP Regeneration",
          "+3.0 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 176,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          600
      ],
      "cooldown": [
          28.0
      ],
      "manaCost": [
          125.0
      ],
      "channelTime": [
          2.5
      ],
      "sharedCooldown": "",
      "cost": 2500,
      "aliases": "mh;meteor hammer",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 21
      },
      {
          "index": 2,
          "componentId": 23
      },
      {
          "index": 3,
          "componentId": 27
      },
      {
          "index": 4,
          "componentId": 28
      }
  ],
  "image": "meteor_hammer_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"224": {
  "id": 224,
  "name": "item_recipe_nullifier",
  "shortName": "recipe_nullifier",
  "language": {
      "displayName": "Nullifier Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"225": {
  "id": 225,
  "name": "item_nullifier",
  "displayName": "Nullifier",
  "shortName": "nullifier",
  "language": {
      "displayName": "Nullifier",
      "lore": [
          "A dangerous weapon pilfered from an inverse dimension."
      ],
      "description": [
          "<h1>Active: Nullify</h1> Dispels the target and mutes them for 6.0 seconds. Anytime the muted target is attacked, it will be slowed by 100 for 0.4 seconds.<br><br>Dispel Type: <span class=\\\"GameplayValues GameplayVariable\\\">Basic Dispel</span>"
      ],
      "notes": [],
      "attributes": [
          "+65 Damage",
          "+5 Armor",
          "+6 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 32,
      "fightLevelRecap": 1,
      "castRange": [
          600
      ],
      "cooldown": [
          13.0
      ],
      "manaCost": [
          75.0
      ],
      "sharedCooldown": "",
      "cost": 4700,
      "aliases": "nlf;nullifier",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 54
      },
      {
          "index": 2,
          "componentId": 6
      }
  ],
  "image": "nullifier_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"226": {
  "id": 226,
  "name": "item_lotus_orb",
  "displayName": "Lotus Orb",
  "shortName": "lotus_orb",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Lotus Orb",
      "lore": [
          "The jewel at its center still reflects a pale image of its creator."
      ],
      "description": [
          "<h1>Active: Echo Shell</h1>Applies a shield to the target unit for 6 seconds which re-casts most targeted spells back to their caster.<br><br>The shielded unit will still take damage from the spell. <br><br>Range: %abilitycastrange%<br>Dispel Type: <span class=\\\"GameplayValues GameplayVariable\\\">Basic Dispel</span>"
      ],
      "notes": [],
      "attributes": [
          "+10 Armor",
          "+6.5 HP Regeneration",
          "+4.0 Mana Regeneration",
          "+250 Mana"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          900
      ],
      "cooldown": [
          15.0
      ],
      "manaCost": [
          75.0
      ],
      "sharedCooldown": "",
      "cost": 4000,
      "shopTags": "regen_health;regen_mana;str;agi;int;hard_to_tag",
      "aliases": "ls;lotus orb",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 69
      },
      {
          "index": 2,
          "componentId": 9
      },
      {
          "index": 3,
          "componentId": 59
      }
  ],
  "image": "lotus_orb_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"227": {
  "id": 227,
  "name": "item_recipe_solar_crest",
  "shortName": "recipe_solar_crest",
  "language": {
      "displayName": "Recipe: Solar Crest",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 400,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"228": {
  "id": 228,
  "name": "item_recipe_octarine_core",
  "shortName": "recipe_octarine_core",
  "language": {
      "displayName": "Recipe: Octarine Core",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"229": {
  "id": 229,
  "name": "item_solar_crest",
  "displayName": "Solar Crest",
  "shortName": "solar_crest",
  "language": {
      "displayName": "Solar Crest",
      "lore": [
          "A talisman forged to honor the daytime sky."
      ],
      "description": [
          "<h1>Active: Shine</h1>When cast on an ally, grants them 8 armor, 80 attack speed, and 10 movement speed. When cast on an enemy, removes 8 armor, 80 attack speed, and 10 movement speed.<br><br>Removes the armor from the caster when used. <br><br> Cannot target spell immune enemies.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+8 Armor",
          "+10 All Attributes",
          "+20 Movement Speed",
          "+1.5 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 4,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          1000
      ],
      "cooldown": [
          7.0
      ],
      "sharedCooldown": "medallion",
      "cost": 3900,
      "shopTags": "armor;regen_mana;hard_to_tag",
      "aliases": "solar crest",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 187
      },
      {
          "index": 2,
          "componentId": 24
      },
      {
          "index": 3,
          "componentId": 244
      }
  ],
  "image": "solar_crest_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"230": {
  "id": 230,
  "name": "item_recipe_guardian_greaves",
  "shortName": "recipe_guardian_greaves",
  "language": {
      "displayName": "Guardian Greaves Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1600,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"231": {
  "id": 231,
  "name": "item_guardian_greaves",
  "displayName": "Guardian Greaves",
  "shortName": "guardian_greaves",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Guardian Greaves",
      "lore": [
          "One of many holy instruments constructed to honor the Omniscience."
      ],
      "description": [
          "<h1>Active: Mend</h1>Restores 300 health and 200 mana to nearby allies, and removes most negative effects from the caster.<br><br>Radius: 1200<br>Dispel Type: <span class=\\\"GameplayValues GameplayVariable\\\">Basic Dispel</span>",
          "<h1>Passive: Guardian Aura</h1> Provides 2.5 health regeneration to allied units. If an allied hero's health falls below 20%, they receive 16 health regeneration and 10 armor. <br><br>Radius: 1200 ",
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [
          "The aura boost only applies to heroes."
      ],
      "attributes": [
          "+17% Movement Speed",
          "+5 All Attributes",
          "+250 Mana",
          "+6 Armor"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          1200
      ],
      "cooldown": [
          40.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 4950,
      "shopTags": "int;armor;regen_health;hard_to_tag",
      "aliases": "guardian greaves",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isAlertable": true,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 79
      },
      {
          "index": 2,
          "componentId": 180
      }
  ],
  "image": "guardian_greaves_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": false
},
"232": {
  "id": 232,
  "name": "item_aether_lens",
  "displayName": "Aether Lens",
  "shortName": "aether_lens",
  "language": {
      "displayName": "Aether Lens",
      "lore": [
          "Polished with the incantation of his final breath, the gift of a dying mage to his sickly son."
      ],
      "description": [
          "<h1>Passive: Aethereal Focus</h1> Increases targeted spell and item cast range by 250."
      ],
      "notes": [
          "Passive does not stack."
      ],
      "attributes": [
          "+450 Mana",
          "+3.0 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2350,
      "shopTags": "int;regen_mana;move_speed;hard_to_tag",
      "aliases": "aether lens",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 59
      },
      {
          "index": 2,
          "componentId": 57
      }
  ],
  "image": "aether_lens_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"233": {
  "id": 233,
  "name": "item_recipe_aether_lens",
  "shortName": "recipe_aether_lens",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Aether Lens Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 600,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"234": {
  "id": 234,
  "name": "item_recipe_dragon_lance",
  "shortName": "recipe_dragon_lance",
  "language": {
      "displayName": "Recipe: Dragon Lance",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"235": {
  "id": 235,
  "name": "item_octarine_core",
  "displayName": "Octarine Core",
  "shortName": "octarine_core",
  "language": {
      "displayName": "Octarine Core",
      "lore": [
          "At the core of spellcraft are spectrums only the very gifted can sense."
      ],
      "description": [
          "<h1>Passive: Cooldown Reduction</h1>Reduces the cooldown time of all spells and items by 25%.",
          "<h1>Passive: Spell Lifesteal</h1>25% of spell damage dealt to enemy heroes is returned to the caster as health, regardless of spell damage type.  <br><br>Lifesteal reduced to 5% against creeps."
      ],
      "notes": [],
      "attributes": [
          "+25 Intelligence",
          "+425 Health",
          "+425 Mana"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 5900,
      "shopTags": "move_speed;boost_mana;mana_pool",
      "aliases": "mana;mb;octarine core",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 58
      },
      {
          "index": 2,
          "componentId": 129
      }
  ],
  "image": "octarine_core_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"236": {
  "id": 236,
  "name": "item_dragon_lance",
  "displayName": "Dragon Lance",
  "shortName": "dragon_lance",
  "language": {
      "displayName": "Dragon Lance",
      "lore": [
          "The forward charge of the wyvern host grants no quarter."
      ],
      "description": [
          "<h1>Passive: Dragon's Reach</h1> Increases attack range of ranged heroes by 140."
      ],
      "notes": [
          "Passive does not stack."
      ],
      "attributes": [
          "+12 Agility",
          "+12 Strength"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1900,
      "shopTags": "damage",
      "aliases": "dragon lance",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 21
      },
      {
          "index": 2,
          "componentId": 18
      },
      {
          "index": 3,
          "componentId": 18
      }
  ],
  "image": "dragon_lance_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"237": {
  "id": 237,
  "name": "item_faerie_fire",
  "displayName": "Faerie Fire",
  "shortName": "faerie_fire",
  "language": {
      "displayName": "Faerie Fire",
      "lore": [
          "The ethereal flames from the ever-burning ruins of Kindertree ignite across realities."
      ],
      "description": [
          "<h1>Use: Imbue</h1> Instantly restores 85 health."
      ],
      "notes": [],
      "attributes": [
          "+2 Damage"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castPoint": [
          0.0
      ],
      "cooldown": [
          5.0
      ],
      "sharedCooldown": "",
      "cost": 70,
      "shopTags": "damage",
      "aliases": "faerie fire",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": true,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "faerie_fire_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"238": {
  "id": 238,
  "name": "item_recipe_iron_talon",
  "displayName": "Recipe: Iron Talon1",
  "shortName": "recipe_iron_talon",
  "language": {
      "displayName": "Iron Talon Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "image": "iron_talon_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"239": {
  "id": 239,
  "name": "item_iron_talon",
  "displayName": "Iron Talon",
  "shortName": "iron_talon",
  "language": {
      "displayName": "Iron Talon",
      "lore": [
          "A simple but effective weapon devised to quell a great Hellbear uprising."
      ],
      "description": [
          "<h1>Active: Chop</h1>Targets a non-player enemy unit to remove 50% of its current HP.<br><br>If cast on a tree or ward, instantly destroys it.<br><br>Unit Range: %abilitycastrange%<br>Tree Range: %abilitycastrange%<BR>Ward Range: 450",
          "<h1>Passive: Quell</h1>Increases attack damage against non-hero units by 18 for melee heroes, and 5 for ranged."
      ],
      "notes": [
          "Has a 4 second cooldown on Trees.",
          "Cannot target ancients."
      ],
      "attributes": [
          "% ARMOR:"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 72,
      "unitTargetTeam": 8,
      "unitTargetFlags": 512,
      "fightLevelRecap": 0,
      "castRange": [
          350
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          25.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 301,
      "shopTags": "damage",
      "aliases": "quelling blade",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 11
      },
      {
          "index": 2,
          "componentId": 12
      }
  ],
  "image": "iron_talon_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"240": {
  "id": 240,
  "name": "item_blight_stone",
  "displayName": "Blight Stone",
  "shortName": "blight_stone",
  "language": {
      "displayName": "Blight Stone",
      "lore": [
          "An unnerving stone unearthed beneath the Fields of Endless Carnage."
      ],
      "description": [
          "<h1>Passive: Lesser Corruption</h1> Your attacks reduce the target's armor by -2 for 8.0 seconds."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 300,
      "shopTags": "hard_to_tag",
      "aliases": "blight stone",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "blight_stone_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"241": {
  "id": 241,
  "name": "item_tango_single",
  "displayName": "Tango (Shared)",
  "shortName": "tango_single",
  "language": {
      "displayName": "Tango (Shared)",
      "lore": [
          "Om nom nom."
      ],
      "description": [
          "<h1>Use: Devour</h1> Consumes a target tree to gain 7.0 health regeneration for 16.0 seconds. Consuming a ward or Ironwood Tree doubles the heal amount.<br><br>Tree Range: %abilitycastrange%<br>Ward Range: 450"
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 72,
      "unitTargetTeam": 8,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          165
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          60.0
      ],
      "sharedCooldown": "",
      "cost": 30,
      "shopTags": "consumable",
      "quality": "consumable",
      "isSellable": false,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "tango_single_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"242": {
  "id": 242,
  "name": "item_crimson_guard",
  "displayName": "Crimson Guard",
  "shortName": "crimson_guard",
  "language": {
      "displayName": "Crimson Guard",
      "lore": [
          "A cuirass originally built to protect against the dreaded Year Beast."
      ],
      "description": [
          "<h1>Active: Guard</h1> For 12 seconds, grant nearby allied heroes and buildings a 100% chance to block 60 damage from each incoming attack.<br><br>Units may only be affected by Guard once every 46 seconds.<br><br>Radius: 1200",
          "<h1>Passive: Damage Block</h1> Grants a 50% chance to block 70 damage from incoming attacks on melee heroes, and 35 damage on ranged."
      ],
      "notes": [
          "Multiple sources of damage block do not stack."
      ],
      "attributes": [
          "+250 Health",
          "+10.5 HP Regeneration",
          "+3 All Attributes",
          "+6 Armor"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "cooldown": [
          46.0
      ],
      "sharedCooldown": "",
      "cost": 3800,
      "shopTags": "armor;boost_armor;regen_health;block;health_pool",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isAlertable": true,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 125
      },
      {
          "index": 2,
          "componentId": 6
      }
  ],
  "image": "crimson_guard_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"243": {
  "id": 243,
  "name": "item_recipe_crimson_guard",
  "shortName": "recipe_crimson_guard",
  "language": {
      "displayName": "Crimson Guard Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 750,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"244": {
  "id": 244,
  "name": "item_wind_lace",
  "displayName": "Wind Lace",
  "shortName": "wind_lace",
  "language": {
      "displayName": "Wind Lace",
      "lore": [
          "Hasten to battle on wind-touched heels."
      ],
      "description": [
          "Bonuses from multiple Wind Laces do not stack."
      ],
      "notes": [],
      "attributes": [
          "+20 Movement Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 250,
      "shopTags": "armor",
      "aliases": "wind lace",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": true,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "wind_lace_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"245": {
  "id": 245,
  "name": "item_recipe_bloodthorn",
  "shortName": "recipe_bloodthorn",
  "language": {
      "displayName": "Bloodthorn Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1000,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"246": {
  "id": 246,
  "name": "item_recipe_moon_shard",
  "shortName": "recipe_moon_shard",
  "language": {
      "displayName": "Recipe: Moon Shard",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"247": {
  "id": 247,
  "name": "item_moon_shard",
  "displayName": "Moon Shard",
  "shortName": "moon_shard",
  "language": {
      "displayName": "Moon Shard",
      "lore": [
          "Said to be a tear from the lunar goddess Selemene."
      ],
      "description": [
          "<h1>Use: Consume</h1> Consume the Moon Shard to permanently gain 60 attack speed and 200 bonus night vision. Max 1 use.",
          "<h1>Passive: Shade Sight</h1>Grants 400 bonus night vision."
      ],
      "notes": [
          "The bonus night vision works during Night Stalker's Darkness ability."
      ],
      "attributes": [
          "+140 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 2056,
      "unitTargetType": 9,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 4000,
      "shopTags": "attack_speed",
      "aliases": "moon shard",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 55
      },
      {
          "index": 2,
          "componentId": 55
      }
  ],
  "image": "moon_shard_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"248": {
  "id": 248,
  "name": "item_recipe_silver_edge",
  "shortName": "recipe_silver_edge",
  "language": {
      "displayName": "Silver Edge Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 600,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"249": {
  "id": 249,
  "name": "item_silver_edge",
  "displayName": "Silver Edge",
  "shortName": "silver_edge",
  "language": {
      "displayName": "Silver Edge",
      "lore": [
          "Once used to slay an unjust king, only to have the kingdom erupt into civil war in the aftermath."
      ],
      "description": [
          "<h1>Active: Shadow Walk</h1>Makes you invisible for 14.0 seconds, or until you attack or cast a spell.  While invisible, you move 25 faster and can move through units.  <br><br>Attacking to end the invisibility will deal 175 bonus physical damage to the target, and for 4 seconds, disable their passive abilities."
      ],
      "notes": [
          "Backstab cannot miss.",
          "Backstab does not pierce spell immunity."
      ],
      "attributes": [
          "+40 Damage",
          "+30 Attack Speed",
          "+15 All Attributes"
      ]
  },
  "stat": {
      "behavior": 526340,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "cooldown": [
          22.0
      ],
      "manaCost": [
          75.0
      ],
      "sharedCooldown": "shadow_blade",
      "cost": 5550,
      "shopTags": "damage;attack_speed;movespeed;hard_to_tag",
      "aliases": "sb;invis;shadow blade",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 152
      },
      {
          "index": 2,
          "componentId": 24
      }
  ],
  "image": "silver_edge_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"250": {
  "id": 250,
  "name": "item_bloodthorn",
  "displayName": "Bloodthorn",
  "shortName": "bloodthorn",
  "language": {
      "displayName": "Bloodthorn",
      "lore": [
          "A reviled blade that bites deeper with each wriggle of its victim's final throes."
      ],
      "description": [
          "<h1>Active: Soul Rend</h1> Silences a target for 5 seconds. At the end of the silence, an additional 30% of all damage taken during the silence will be dealt to the target as magical damage.<br><br>All attacks on the silenced target will have True Strike and 100 chance to crit for 140 damage.<br><br>Range: %abilitycastrange%",
          "<h1>Passive: Critical Strike</h1> Grants a 20% chance for attacks to inflict 175% damage."
      ],
      "notes": [
          "Critical Strike does not work against buildings."
      ],
      "attributes": [
          "+25 Intelligence",
          "+30 Attack Speed",
          "+68 Damage",
          "+5.5 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          900
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          15.0
      ],
      "manaCost": [
          100.0
      ],
      "sharedCooldown": "orchid",
      "cost": 7055,
      "shopTags": "int;attack_speed;damage;regen_mana;damage;hard_to_tag",
      "aliases": "orchid malevolence",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 98
      },
      {
          "index": 2,
          "componentId": 149
      }
  ],
  "image": "bloodthorn_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"251": {
  "id": 251,
  "name": "item_recipe_echo_sabre",
  "shortName": "recipe_echo_sabre",
  "language": {
      "displayName": "Recipe: Echo Sabre",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"252": {
  "id": 252,
  "name": "item_echo_sabre",
  "displayName": "Echo Sabre",
  "shortName": "echo_sabre",
  "language": {
      "displayName": "Echo Sabre",
      "lore": [
          "A deceptively swift blade imbued with resonant magic."
      ],
      "description": [
          "<h1>Passive: Echo Strike</h1> Causes melee attacks to attack twice in quick succession. The double attacks apply a 100% movement slow for 0.8 seconds on each strike."
      ],
      "notes": [],
      "attributes": [
          "+10 Intelligence",
          "+12 Strength",
          "+10 Attack Speed",
          "+15 Damage",
          "+1.25 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castPoint": [
          0.0
      ],
      "cooldown": [
          5.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 2575,
      "shopTags": "int;attack_speed;damage;regen_mana;damage;hard_to_tag",
      "aliases": "echo sabre",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 21
      },
      {
          "index": 2,
          "componentId": 67
      }
  ],
  "image": "echo_sabre_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"253": {
  "id": 253,
  "name": "item_recipe_glimmer_cape",
  "shortName": "recipe_glimmer_cape",
  "language": {
      "displayName": "Recipe: Glimmer Cape",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"254": {
  "id": 254,
  "name": "item_glimmer_cape",
  "displayName": "Glimmer Cape",
  "shortName": "glimmer_cape",
  "isSupportFullItem": true,
  "language": {
      "displayName": "Glimmer Cape",
      "lore": [
          "The stolen cape of a master illusionist."
      ],
      "description": [
          "<h1>Active: Glimmer</h1> After a 0.6 second delay, grants invisibility and 45% magic resistance to you or a target allied unit for 5 seconds.<br><br>Range: %abilitycastrange%<br><br>Can be cast while channelling."
      ],
      "notes": [],
      "attributes": [
          "+20 Attack Speed",
          "+15% Magic Resistance"
      ]
  },
  "stat": {
      "behavior": 534536,
      "unitTargetType": 3,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          800
      ],
      "cooldown": [
          14.0
      ],
      "manaCost": [
          90.0
      ],
      "sharedCooldown": "",
      "cost": 1950,
      "shopTags": "",
      "aliases": "glimmer cape",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 215
      },
      {
          "index": 2,
          "componentId": 31
      }
  ],
  "image": "glimmer_cape_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"255": {
  "id": 255,
  "name": "item_recipe_aeon_disk",
  "shortName": "recipe_aeon_disk",
  "language": {
      "displayName": "Aeon Disk Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1100,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"256": {
  "id": 256,
  "name": "item_aeon_disk",
  "displayName": "Aeon Disk",
  "shortName": "aeon_disk",
  "language": {
      "displayName": "Aeon Disk",
      "lore": [
          "A powerful artifact long ago smuggled out of the Ivory Incubarium. Or so many believe."
      ],
      "description": [
          "<h1>Passive: Combo Breaker</h1> When you take damage and your health falls below 70, a strong dispel is applied and you gain a 2.5 second buff that provides +75 Status Resistance and causes all damage you deal and are dealt to be reduced to zero. Only triggers on player based damage."
      ],
      "notes": [],
      "attributes": [
          "+300 Health",
          "+300 Mana"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          115.0
      ],
      "sharedCooldown": "",
      "cost": 3100,
      "shopTags": "str;regen_health;health_pool",
      "aliases": "ad;aeon disk",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 61
      },
      {
          "index": 2,
          "componentId": 59
      }
  ],
  "image": "aeon_disk_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"257": {
  "id": 257,
  "name": "item_tome_of_knowledge",
  "displayName": "Tome of Knowledge",
  "shortName": "tome_of_knowledge",
  "language": {
      "displayName": "Tome of Knowledge",
      "lore": [
          "That which raises beast to man and man to god."
      ],
      "description": [
          "<h1>Use: Enlighten</h1>Grants you 700 experience plus 135 per tome consumed by your team after the first two.<br><br>Tomes Used By Team: %customval_team_tomes_used%"
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 34820,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "sharedCooldown": "tome",
      "cost": 150,
      "shopTags": "consumable",
      "aliases": "tome of knowledge",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": true,
      "isSupport": true,
      "isTempestDoubleClonable": false,
      "stockMax": 3,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 600,
      "initialStockTime": 690,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "tome_of_knowledge_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"258": {
  "id": 258,
  "name": "item_recipe_kaya",
  "shortName": "recipe_kaya",
  "language": {
      "displayName": "Kaya Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 600,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"259": {
  "id": 259,
  "name": "item_kaya",
  "displayName": "Kaya",
  "shortName": "kaya",
  "language": {
      "displayName": "Kaya",
      "lore": [
          "The staff of a renowned sorceress, lost for countless millennia."
      ],
      "description": [],
      "notes": [
          "Multiple instances of this item do not stack."
      ],
      "attributes": [
          "+16 Intelligence",
          "+8% Spell Amplification",
          "+12% Manacost and Manaloss Reduction"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 2050,
      "shopTags": "int;mana",
      "aliases": "ky;kaya",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 23
      },
      {
          "index": 2,
          "componentId": 19
      }
  ],
  "image": "kaya_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"260": {
  "id": 260,
  "name": "item_refresher_shard",
  "displayName": "Refresher Shard",
  "shortName": "refresher_shard",
  "language": {
      "displayName": "Refresher Shard",
      "lore": [
          "Across an epoch of endless battle, the Immortal's will to triumph never faltered."
      ],
      "description": [
          "<h1>Use: Reset Cooldowns</h1>Resets the cooldowns of all your items and abilities."
      ],
      "notes": [
          "Refresher Shard is shareable."
      ],
      "attributes": []
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 2,
      "cooldown": [
          200.0
      ],
      "sharedCooldown": "",
      "cost": 1000,
      "shopTags": "",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "refresher_shard_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"261": {
  "id": 261,
  "name": "item_crown",
  "displayName": "Crown",
  "shortName": "crown",
  "language": {
      "displayName": "Crown",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": [
          "+4 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 450,
      "shopTags": "agi;int;str",
      "aliases": "crown",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "crown_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"262": {
  "id": 262,
  "name": "item_recipe_hurricane_pike",
  "shortName": "recipe_hurricane_pike",
  "language": {
      "displayName": "Hurricane Pike Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 450,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"263": {
  "id": 263,
  "name": "item_hurricane_pike",
  "displayName": "Hurricane Pike",
  "shortName": "hurricane_pike",
  "language": {
      "displayName": "Hurricane Pike",
      "lore": [
          "A legendary pike once held as royal sigil of the ancient wyvern riders."
      ],
      "description": [
          "<h1>Active: Hurricane Thrust</h1> Pushes you and target enemy 450 units away from each other, and for 5 seconds, allows you to make 4 attacks against the target without range restrictions and with +100 attack speed.<br><br>Works like Force Staff when used on self or allies.<br><br>Allied Range: %abilitycastrange%<br>Enemy Range: 400",
          "<h1>Passive:  Dragon's Reach</h1> Increases attack range of ranged heroes by 140."
      ],
      "notes": [
          "Self-cast will use Hurricane Pike on yourself.",
          "Hurricane Pike doesn't interrupt the target's actions.",
          "Will not work on a unit inside Chronosphere, Duel, or Black Hole."
      ],
      "attributes": [
          "+13 Intelligence",
          "+2.5 HP Regeneration",
          "+20 Agility",
          "+15 Strength"
      ]
  },
  "stat": {
      "behavior": 32776,
      "unitTargetType": 11,
      "unitTargetTeam": 12,
      "unitTargetFlags": 0,
      "fightLevelRecap": 1,
      "castRange": [
          800
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          23.0
      ],
      "manaCost": [
          100.0
      ],
      "sharedCooldown": "force",
      "cost": 4600,
      "shopTags": "int;damage;attack_speed;hard_to_tag",
      "aliases": "fs;force staff",
      "quality": "epic",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 102
      },
      {
          "index": 2,
          "componentId": 236
      }
  ],
  "image": "hurricane_pike_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"265": {
  "id": 265,
  "name": "item_infused_raindrop",
  "displayName": "Infused Raindrops",
  "shortName": "infused_raindrop",
  "language": {
      "displayName": "Infused Raindrops",
      "lore": [
          "Elemental protection from magical assaults."
      ],
      "description": [
          "<h1>Passive: Magical Damage Block</h1> Consumes a charge to block 120 magic damage from damage instances over 50 damage. <br><br>Comes with 6 charges. When the charges are gone, the item disappears."
      ],
      "notes": [
          "Uses at most one charge per damage instance."
      ],
      "attributes": [
          "+0.75 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castPoint": [
          0.0
      ],
      "cooldown": [
          7.0
      ],
      "sharedCooldown": "",
      "cost": 225,
      "shopTags": "armor",
      "aliases": "raindrop",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 6,
      "initialStock": 0,
      "stockTime": 1,
      "initialStockTime": 270,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "infused_raindrop_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"266": {
  "id": 266,
  "name": "item_recipe_spirit_vessel",
  "shortName": "recipe_spirit_vessel",
  "language": {
      "displayName": "Spirit Vessel Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 750,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"267": {
  "id": 267,
  "name": "item_spirit_vessel",
  "displayName": "Spirit Vessel",
  "shortName": "spirit_vessel",
  "language": {
      "displayName": "Spirit Vessel",
      "lore": [
          "Forged by a god to entrap the soul of another."
      ],
      "description": [
          "<h1>Active: Soul Release</h1> When used against enemies, it reduces health by 4% of current health per second, and reduces HP regeneration and healing by 60. Deals 25 damage per second. <br><br>When used on allies, it provides 30 health regeneration per second. <br><br>Lasts 8.0 seconds.  <br><br>Gains charges every time an enemy hero dies within 1400 units.  Only the closest Spirit Vessel to the dying hero will gain a charge.<br><br>Cast Range: %abilitycastrange% "
      ],
      "notes": [],
      "attributes": [
          "+250 Health",
          "+20 Movement Speed",
          "+1.5 Mana Regeneration",
          "+2 All Attributes",
          "+2 Armor"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 1,
      "unitTargetTeam": 4,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          950
      ],
      "cooldown": [
          7.0
      ],
      "sharedCooldown": "urn",
      "cost": 2965,
      "aliases": "sv;spirit vessel",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": true,
      "isHideCharges": false,
      "isRequiresCharges": true,
      "isDisplayCharges": true,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 92
      },
      {
          "index": 2,
          "componentId": 61
      },
      {
          "index": 3,
          "componentId": 244
      }
  ],
  "image": "spirit_vessel_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"268": {
  "id": 268,
  "name": "item_recipe_holy_locket",
  "shortName": "recipe_holy_locket",
  "language": {
      "displayName": "Holy Locket Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 850,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"269": {
  "id": 269,
  "name": "item_holy_locket",
  "displayName": "Holy Locket",
  "shortName": "holy_locket",
  "language": {
      "displayName": "Holy Locket",
      "lore": [],
      "description": [
          "<h1>Active: Energy Charge</h1>Instantly restores 15 health and mana per charge stored. <br><br>Max 20 charges. Gains a charge whenever a visible enemy within 1200 range uses an ability.  ",
          "",
          " <h1>Passive: Holy Blessing</h1>Amplifies heals you provide by 35%."
      ],
      "notes": [],
      "attributes": [
          "+250 Health",
          "+325 Mana",
          "+4 HP Regeneration",
          "+3 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1200
      ],
      "cooldown": [
          13.0
      ],
      "sharedCooldown": "magicwand",
      "cost": 2900,
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 279
      },
      {
          "index": 2,
          "componentId": 59
      },
      {
          "index": 3,
          "componentId": 36
      }
  ],
  "image": "holy_locket_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"270": {
  "id": 270,
  "name": "item_recipe_ultimate_scepter_2",
  "shortName": "recipe_ultimate_scepter_2",
  "language": {
      "displayName": "Recipe: ",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1800,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"271": {
  "id": 271,
  "name": "item_ultimate_scepter_2",
  "displayName": "Aghanim's Blessing",
  "shortName": "ultimate_scepter_2",
  "language": {
      "displayName": "Recipe: ",
      "lore": [
          "The scepter of a wizard with demigod-like powers."
      ],
      "description": [
          "<h1>Passive: Ability Upgrade</h1>Upgrades the ultimate, and some abilities, of all heroes."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 6000,
      "shopTags": "int;str;agi;mana_pool;health_pool;hard_to_tag",
      "aliases": "ags;ultimate;aghanim's scepter;ags",
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 108
      }
  ],
  "image": "ultimate_scepter_2_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"272": {
  "id": 272,
  "name": "item_recipe_kaya_and_sange",
  "shortName": "recipe_kaya_and_sange",
  "language": {
      "displayName": "Kaya and Sange Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"273": {
  "id": 273,
  "name": "item_kaya_and_sange",
  "displayName": "Kaya and Sange",
  "shortName": "kaya_and_sange",
  "language": {
      "displayName": "Kaya and Sange",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": [
          "+16 Strength",
          "+16 Intelligence",
          "+12% Spell Amplification",
          "+16% Manacost and Manaloss Reduction",
          "+20% Status Resistance",
          "+30% Self HP Regen and Lifesteal Amp"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 4100,
      "shopTags": "damage;str;agi;attack_speed;unique",
      "aliases": "snk;s&k;sk;sange and kaya",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 162
      },
      {
          "index": 2,
          "componentId": 259
      }
  ],
  "image": "kaya_and_sange_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"274": {
  "id": 274,
  "name": "item_recipe_yasha_and_kaya",
  "shortName": "recipe_yasha_and_kaya",
  "language": {
      "displayName": "Yasha and Kaya Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"275": {
  "id": 275,
  "name": "item_recipe_trident",
  "displayName": "DOTA_Tooltip_Ability_item_trident",
  "shortName": "recipe_trident",
  "language": {
      "displayName": "Trident Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "image": "trident_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"276": {
  "id": 276,
  "name": "item_combo_breaker",
  "displayName": "DOTA_Tooltip_Ability_item_combo_breaker",
  "shortName": "combo_breaker",
  "language": {
      "displayName": "Recipe: ",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "combo_breaker_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"277": {
  "id": 277,
  "name": "item_yasha_and_kaya",
  "displayName": "Yasha and Kaya",
  "shortName": "yasha_and_kaya",
  "language": {
      "displayName": "Yasha and Kaya",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": [
          "+16 Intelligence",
          "+12% Spell Amplification",
          "+16% Manacost and Manaloss Reduction",
          "+16 Agility",
          "+16 Attack Speed",
          "+30 Movement Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 4100,
      "shopTags": "damage;str;agi;attack_speed;unique",
      "aliases": "ynk;y&k;sk;yasha and kaya",
      "quality": "artifact",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 259
      },
      {
          "index": 2,
          "componentId": 170
      }
  ],
  "image": "yasha_and_kaya_lg.png?3",
  "isInNeuralNetwork": true,
  "isFullItemHeroPurchaseItem": true
},
"279": {
  "id": 279,
  "name": "item_ring_of_tarrasque",
  "displayName": "Ring of Tarrasque",
  "shortName": "ring_of_tarrasque",
  "language": {
      "displayName": "Ring of Tarrasque",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": [
          "+3.75 HP Regeneration",
          "+150 Health"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 700,
      "shopTags": "regen_health",
      "aliases": "rot",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ring_of_tarrasque_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"286": {
  "id": 286,
  "name": "item_flying_courier",
  "displayName": "Flying Courier",
  "shortName": "flying_courier",
  "language": {
      "displayName": "Flying Courier",
      "lore": [
          "The faithful mount upon which soar the fortunes of battle."
      ],
      "description": [
          "<h1>Use: Upgrade Courier</h1>Upgrades your team's Animal Courier to a Flying Courier, granting it swift, unobstructed movement to carry items to and from your team's base. <br><br>Requires a deployed Animal Courier."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 4,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 100,
      "shopTags": "consumable",
      "aliases": "crow;flying courier",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": true,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 1,
      "initialStockTime": 270,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "flying_courier_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"287": {
  "id": 287,
  "name": "item_keen_optic",
  "displayName": "Keen Optic",
  "shortName": "keen_optic",
  "language": {
      "displayName": "Keen Optic",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": [
          "+1 Mana Regeneration",
          "+75 Cast Range"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "keen_optic_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"288": {
  "id": 288,
  "name": "item_grove_bow",
  "displayName": "Grove Bow",
  "shortName": "grove_bow",
  "language": {
      "displayName": "Grove Bow",
      "lore": [],
      "description": [
          "<h1>Passive: Magic Amp</h1> Reduces magic resistance of the attacked enemy by 12%."
      ],
      "notes": [],
      "attributes": [
          "+100 Attack Range <span className={s.abilityitemgrovebow}>(Ranged Only)</span>",
          "+10 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "grove_bow_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"289": {
  "id": 289,
  "name": "item_quickening_charm",
  "displayName": "Quickening Charm",
  "shortName": "quickening_charm",
  "language": {
      "displayName": "Quickening Charm",
      "lore": [],
      "description": [
          "<h1>Passive: Cooldown Reduction</h1>Reduces the cooldown time of all spells and items by 13%"
      ],
      "notes": [],
      "attributes": [
          "+9 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "quickening_charm_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"290": {
  "id": 290,
  "name": "item_philosophers_stone",
  "displayName": "Philosopher's Stone",
  "shortName": "philosophers_stone",
  "language": {
      "displayName": "Philosopher's Stone",
      "lore": [],
      "description": [
          "Increases gold income and mana, while reducing outgoing attack damage."
      ],
      "notes": [],
      "attributes": [
          "+60 GPM BONUS",
          "+200 Mana",
          "35% -<span className={s.abilityitemphilosophersstone}>Attack Damage</span>"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "philosophers_stone_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"291": {
  "id": 291,
  "name": "item_force_boots",
  "displayName": "Force Boots",
  "shortName": "force_boots",
  "language": {
      "displayName": "Force Boots",
      "lore": [],
      "description": [
          "<h1>Active: Force</h1>Dispels the user and pushes them 800 units in the direction they are facing.",
          " <h1>Passive: Speed Unlock</h1> Completely removes the speed limit for the wearer ",
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [],
      "attributes": [
          "+50% Movement Speed"
      ]
  },
  "stat": {
      "behavior": 526340,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          6.0
      ],
      "manaCost": [
          75.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "force_boots_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"292": {
  "id": 292,
  "name": "item_desolator_2",
  "displayName": "Desolator 2",
  "shortName": "desolator_2",
  "language": {
      "displayName": "Recipe: ",
      "lore": [],
      "description": [
          "<h1>Passive: Greater Corruption</h1> Your attacks reduce the target's armor by -12 for 15.0 seconds."
      ],
      "notes": [
          "Armor reduction works on buildings."
      ],
      "attributes": [
          "+100 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "desolator_2_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"293": {
  "id": 293,
  "name": "item_phoenix_ash",
  "displayName": "Phoenix Ash",
  "shortName": "phoenix_ash",
  "language": {
      "displayName": "Phoenix Ash",
      "lore": [],
      "description": [
          "<h1>Passive: Rebirth</h1>Lethal damage is prevented and instead the wearer is healed to half health and non-ultimate ability cooldowns reset. Gets consumed on trigger."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "phoenix_ash_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"294": {
  "id": 294,
  "name": "item_seer_stone",
  "displayName": "Seer Stone",
  "shortName": "seer_stone",
  "language": {
      "displayName": "Seer Stone",
      "lore": [],
      "description": [
          "Increases cast range as well as daytime and night time vision."
      ],
      "notes": [],
      "attributes": [
          "+450 Cast Range",
          "+450 Vision Bonus"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "seer_stone_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"295": {
  "id": 295,
  "name": "item_greater_mango",
  "displayName": "Greater Mango",
  "shortName": "greater_mango",
  "language": {
      "displayName": "Greater Mango",
      "lore": [],
      "description": [
          "<h1>Consume Unbelievably Good Mango</h1>Overwhelms your hero, causing you to die. However, when you respawn, you will have all talents on both sides skilled."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castPoint": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "greater_mango_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"297": {
  "id": 297,
  "name": "item_vampire_fangs",
  "displayName": "Vampire Fangs",
  "shortName": "vampire_fangs",
  "language": {
      "displayName": "Vampire Fangs",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": [
          "+15% Lifesteal",
          "+8% Spell Lifesteal",
          "+250 Night Vision"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "vampire_fangs_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"298": {
  "id": 298,
  "name": "item_craggy_coat",
  "displayName": "Craggy Coat",
  "shortName": "craggy_coat",
  "language": {
      "displayName": "Craggy Coat",
      "lore": [],
      "description": [
          "Increases the wearer's armor, while reducing their attack speed."
      ],
      "notes": [],
      "attributes": [
          "+13 ARMOR",
          "35% -<span className={s.abilityitemcraggycoat}>Attack Speed</span>"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "craggy_coat_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"299": {
  "id": 299,
  "name": "item_greater_faerie_fire",
  "displayName": "Greater Faerie Fire",
  "shortName": "greater_faerie_fire",
  "language": {
      "displayName": "Greater Faerie Fire",
      "lore": [],
      "description": [
          "<h1>Use: Imbue</h1> Instantly restores 450 health."
      ],
      "notes": [],
      "attributes": [
          "+35 Damage"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          10.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 3,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "greater_faerie_fire_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"300": {
  "id": 300,
  "name": "item_timeless_relic",
  "displayName": "Timeless Relic",
  "shortName": "timeless_relic",
  "language": {
      "displayName": "Timeless Relic",
      "lore": [],
      "description": [
          "Increases all outgoing spell damage and debuff duration."
      ],
      "notes": [],
      "attributes": [
          "+25% Debuff Duration",
          "+15% Spell Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "timeless_relic_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"301": {
  "id": 301,
  "name": "item_mirror_shield",
  "displayName": "Mirror Shield",
  "shortName": "mirror_shield",
  "language": {
      "displayName": "Mirror Shield",
      "lore": [],
      "description": [
          "<h1>Passive: Echo Shield</h1> Block and reflect most targeted spells back to their caster every once every 4.0 seconds."
      ],
      "notes": [],
      "attributes": [
          "+20 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "mirror_shield_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"302": {
  "id": 302,
  "name": "item_elixer",
  "displayName": "Elixir",
  "shortName": "elixer",
  "language": {
      "displayName": "Elixir",
      "lore": [],
      "description": [
          "<h1>Use: Consume</h1> Restores 500 health and 250 mana to the target over 6 seconds.<br><br>If the unit is attacked by an enemy hero or Roshan, the effect is lost.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          250
      ],
      "castPoint": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 3,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "elixer_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"303": {
  "id": 303,
  "name": "item_recipe_ironwood_tree",
  "shortName": "recipe_ironwood_tree",
  "language": {
      "displayName": "Ironwood Tree Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1,
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"304": {
  "id": 304,
  "name": "item_ironwood_tree",
  "displayName": "Ironwood Tree",
  "shortName": "ironwood_tree",
  "language": {
      "displayName": "Ironwood Tree",
      "lore": [],
      "description": [
          "<h1>Use: Plant Tree</h1> Targets the ground to plant a happy little tree that lasts for 20 seconds.<br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+7 All Attributes"
      ]
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          800
      ],
      "cooldown": [
          4.0
      ],
      "sharedCooldown": "",
      "cost": 151,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 16
      },
      {
          "index": 2,
          "componentId": 16
      },
      {
          "index": 3,
          "componentId": 16
      }
  ],
  "image": "ironwood_tree_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"305": {
  "id": 305,
  "name": "item_royal_jelly",
  "displayName": "Royal Jelly",
  "shortName": "royal_jelly",
  "language": {
      "displayName": "Royal Jelly",
      "lore": [],
      "description": [
          "<h1>Use: Consume</h1> Grants a target allied unit a permanent buff that provides +2.5 Health Regen and +1.25 Mana Regen.<br><br>Does not stack on the same unit."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 16,
      "fightLevelRecap": 0,
      "castRange": [
          250
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 2,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "royal_jelly_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"306": {
  "id": 306,
  "name": "item_pupils_gift",
  "displayName": "Pupil's Gift",
  "shortName": "pupils_gift",
  "language": {
      "displayName": "Pupil's Gift",
      "lore": [],
      "description": [
          "Increases both non-primary attributes."
      ],
      "notes": [],
      "attributes": [
          "+13 SECONDARY STATS"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "pupils_gift_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"307": {
  "id": 307,
  "name": "item_tome_of_aghanim",
  "displayName": "Tome of Aghanim",
  "shortName": "tome_of_aghanim",
  "language": {
      "displayName": "Tome of Aghanim",
      "lore": [],
      "description": [
          "<h1>Use: Consume</h1> Temporarily grants an allied target the Aghanim's Scepter buff for 3 minutes."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          250
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "tome_of_aghanim_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"308": {
  "id": 308,
  "name": "item_repair_kit",
  "displayName": "Repair Kit",
  "shortName": "repair_kit",
  "language": {
      "displayName": "Repair Kit",
      "lore": [],
      "description": [
          "<h1>Use: Building Repair</h1> Targets a building, restoring 40 of it's health over 30 seconds. Also grants +10 armor and +4 Multishot attack during this period.<br><br> Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+13 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 32,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          600
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 3,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "repair_kit_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"309": {
  "id": 309,
  "name": "item_mind_breaker",
  "displayName": "Mind Breaker",
  "shortName": "mind_breaker",
  "language": {
      "displayName": "Mind Breaker",
      "lore": [],
      "description": [
          "<h1>Passive: Silence Strike</h1> The next attack silences the hit enemy for 2 seconds."
      ],
      "notes": [],
      "attributes": [
          "5% MAGIC ATTACK BONUS DAMAGE:",
          "5% ATTACK SPEED:"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          20.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "mind_breaker_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"310": {
  "id": 310,
  "name": "item_third_eye",
  "displayName": "Third Eye",
  "shortName": "third_eye",
  "language": {
      "displayName": "Third Eye",
      "lore": [],
      "description": [
          "<h1>Passive: True Sight</h1> Grants the ability to see invisible units and wards to any allied vision within 500 range of its carrier. ",
          "<h1>Passive: Charge Loss</h1><font color='#e03e2e'>Loses a charge on death. After all charges are lost, the item disappears.</font>"
      ],
      "notes": [],
      "attributes": [
          "+300 BONUS VISION",
          "+7 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 3,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "third_eye_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"311": {
  "id": 311,
  "name": "item_spell_prism",
  "displayName": "Spell Prism",
  "shortName": "spell_prism",
  "language": {
      "displayName": "Spell Prism",
      "lore": [],
      "description": [
          "<h1>Passive: Cooldown Reduction</h1>Reduces the cooldown time of all spells and items by 20%."
      ],
      "notes": [],
      "attributes": [
          "+12 All Attributes",
          "+4 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "spell_prism_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"312": {
  "id": 312,
  "name": "item_horizon",
  "displayName": "Horizon",
  "shortName": "horizon",
  "language": {
      "displayName": "Horizon",
      "lore": [],
      "description": [
          "<h1>Active: Blink Strike</h1> Targets a unit to teleport to them. Knocks enemies back 250 distance and disables them for 1 second. <br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 4104,
      "unitTargetType": 8,
      "unitTargetTeam": 8,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1600
      ],
      "cooldown": [
          10.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "horizon_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"313": {
  "id": 313,
  "name": "item_fusion_rune",
  "displayName": "Fusion Rune",
  "shortName": "fusion_rune",
  "language": {
      "displayName": "Fusion Rune",
      "lore": [],
      "description": [
          "<h1>Use: Consume</h1> Grants the target the bonuses of every Power Rune for 50 seconds. Each use consumes a charge."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 1,
      "unitTargetTeam": 2,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          250
      ],
      "cooldown": [
          120.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 3,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "fusion_rune_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"317": {
  "id": 317,
  "name": "item_recipe_fallen_sky",
  "shortName": "recipe_fallen_sky",
  "language": {
      "displayName": "Recipe: Fallen Sky",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"325": {
  "id": 325,
  "name": "item_princes_knife",
  "displayName": "Prince's Knife",
  "shortName": "princes_knife",
  "language": {
      "displayName": "Prince's Knife",
      "lore": [],
      "description": [
          "<h1>Passive: Hex Strike</h1> The next attack hexes the hit enemy into a frog for 1.5 seconds."
      ],
      "notes": [],
      "attributes": [
          "+60% Attack Projectile Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          12.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "princes_knife_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"326": {
  "id": 326,
  "name": "item_spider_legs",
  "displayName": "Spider Legs",
  "shortName": "spider_legs",
  "language": {
      "displayName": "Spider Legs",
      "lore": [],
      "description": [
          "<h1>Active: Scurry</h1> Grants you 24% bonus movement speed and free pathing for 3 seconds. Walking over trees causes them to be destroyed.",
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [],
      "attributes": [
          "+24% Movement Speed",
          "+30% Turn Rate"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          10.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "spider_legs_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"327": {
  "id": 327,
  "name": "item_helm_of_the_undying",
  "displayName": "Helm of the Undying",
  "shortName": "helm_of_the_undying",
  "language": {
      "displayName": "Helm of the Undying",
      "lore": [],
      "description": [
          "<h1>Passive: Death Delay</h1> Survive for an extra 5 seconds after receiving a killing blow."
      ],
      "notes": [],
      "attributes": [
          "+6 Armor"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "helm_of_the_undying_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"328": {
  "id": 328,
  "name": "item_mango_tree",
  "displayName": "Mango Tree",
  "shortName": "mango_tree",
  "language": {
      "displayName": "Mango Tree",
      "lore": [],
      "description": [
          "<h1>Use: Plant a Mango Tree</h1> Targets the ground to plant a mango tree that provides unlimited mango power. The tree generates Enchanted Mangoes every 60 seconds, and provides unobstructed vision in the area <br><br>Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          200
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "mango_tree_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"329": {
  "id": 329,
  "name": "item_recipe_vambrace",
  "shortName": "recipe_vambrace",
  "language": {
      "displayName": "Vambrace Recipe",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 0,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1,
      "shopTags": "",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": true
  },
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"330": {
  "id": 330,
  "name": "item_witless_shako",
  "displayName": "Witless Shako",
  "shortName": "witless_shako",
  "language": {
      "displayName": "Witless Shako",
      "lore": [],
      "description": [
          "Increases the wearer's max health, while reducing their max mana."
      ],
      "notes": [],
      "attributes": [
          "+1000 Max Health Increase",
          "00% -<span className={s.abilityitemwitlessshako}>Max Mana</span>"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "witless_shako_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"331": {
  "id": 331,
  "name": "item_vambrace",
  "displayName": "Vambrace",
  "shortName": "vambrace",
  "language": {
      "displayName": "Vambrace",
      "lore": [],
      "description": [
          "Also provides one of the following, based on the Vambrace's main attribute:",
          " <font color='#ec3d06'>Strength:</font> +10% Magic Resistance ",
          " <font color='#3ed038'>Agility:</font> +10 Attack Speed ",
          " <font color='#00d9ec'>Intelligence:</font> +6% Spell Amp"
      ],
      "notes": [],
      "attributes": [
          "+12 Main Vambrace Attribute",
          "+6 Other Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 1021,
      "shopTags": "damage;int;agi;str",
      "aliases": "bracer",
      "quality": "common",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 73
      },
      {
          "index": 2,
          "componentId": 73
      }
  ],
  "image": "vambrace_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"334": {
  "id": 334,
  "name": "item_imp_claw",
  "displayName": "Imp Claw",
  "shortName": "imp_claw",
  "language": {
      "displayName": "Imp Claw",
      "lore": [],
      "description": [
          "<h1>Passive: Critical Strike</h1> Empowers your next attack, causing it to be a critical strike dealing 130% damage."
      ],
      "notes": [],
      "attributes": [
          "+24 Damage"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          8.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "imp_claw_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"335": {
  "id": 335,
  "name": "item_flicker",
  "displayName": "Flicker",
  "shortName": "flicker",
  "language": {
      "displayName": "Flicker",
      "lore": [],
      "description": [
          "<h1>Active: Flicker</h1> Dispells debuffs and blinks you to a random spot within 600 range. Does not get disabled on incoming damage."
      ],
      "notes": [],
      "attributes": [
          "+30 Movement Speed"
      ]
  },
  "stat": {
      "behavior": 6148,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          5.0
      ],
      "manaCost": [
          25.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "flicker_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"336": {
  "id": 336,
  "name": "item_spy_gadget",
  "displayName": "Telescope",
  "shortName": "spy_gadget",
  "language": {
      "displayName": "Telescope",
      "lore": [],
      "description": [
          "<h1>Passive: Prescient Aura</h1> Lowers Scan cooldown by 50% . Increases attack and cast range of allies in a 1200 unit radius."
      ],
      "notes": [],
      "attributes": [
          "0% SCAN COOLDOWN REDUCTION:",
          "50% AURA BONUS ATTACK RANGE:",
          "25% AURA BONUS CAST RANGE:"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "spy_gadget_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"349": {
  "id": 349,
  "name": "item_arcane_ring",
  "displayName": "Arcane Ring",
  "shortName": "arcane_ring",
  "language": {
      "displayName": "Arcane Ring",
      "lore": [],
      "description": [
          "<h1>Active: Replenish Mana</h1> Restores 75 mana to all nearby allies. <br><br> Radius: 1200"
      ],
      "notes": [],
      "attributes": [
          "+3 Armor",
          "+8 Intelligence"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          40.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "arcane_ring_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"354": {
  "id": 354,
  "name": "item_ocean_heart",
  "displayName": "Ocean Heart",
  "shortName": "ocean_heart",
  "language": {
      "displayName": "Ocean Heart",
      "lore": [],
      "description": [
          "<h1>Passive: Water Regen</h1> Provides you with 6 HP regen and 3 mana regen while in the river."
      ],
      "notes": [],
      "attributes": [
          "+5 All Attributes"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ocean_heart_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"355": {
  "id": 355,
  "name": "item_broom_handle",
  "displayName": "Broom Handle",
  "shortName": "broom_handle",
  "language": {
      "displayName": "Broom Handle",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": [
          "+35 Attack Range <span className={s.abilityitembroomhandle}>(Melee Only)</span>",
          "+12 Damage",
          "+3 Armor"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "broom_handle_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"356": {
  "id": 356,
  "name": "item_trusty_shovel",
  "displayName": "Trusty Shovel",
  "shortName": "trusty_shovel",
  "language": {
      "displayName": "Trusty Shovel",
      "lore": [],
      "description": [
          "<h1>Active: Dig</h1> Channel for %abilitychanneltime% second. <br>Can find a Bounty Rune, a Flask, a 2 charged TP Scroll bundle, or an enemy Kobold."
      ],
      "notes": [],
      "attributes": [
          "+150 Health"
      ]
  },
  "stat": {
      "behavior": 16528,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          250
      ],
      "castPoint": [
          0.1
      ],
      "cooldown": [
          55.0
      ],
      "channelTime": [
          1.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "trusty_shovel_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"357": {
  "id": 357,
  "name": "item_nether_shawl",
  "displayName": "Nether Shawl",
  "shortName": "nether_shawl",
  "language": {
      "displayName": "Nether Shawl",
      "lore": [],
      "description": [
          "Increases the wearer's magic resistance and magic damage, while decreasing their armor."
      ],
      "notes": [],
      "attributes": [
          "+20% Magic Resistance",
          "+8% Spell Damage",
          "% -<span className={s.abilityitemnethershawl}>Armor</span>"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "nether_shawl_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"358": {
  "id": 358,
  "name": "item_dragon_scale",
  "displayName": "Dragon Scale",
  "shortName": "dragon_scale",
  "language": {
      "displayName": "Dragon Scale",
      "lore": [],
      "description": [
          "<h1>Passive: Afterburn</h1> Causes attacks to burn the enemy, dealing 12 damage per second for 3 seconds. Affects buildings."
      ],
      "notes": [],
      "attributes": [
          "+5 Armor",
          "+5 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "dragon_scale_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"359": {
  "id": 359,
  "name": "item_essence_ring",
  "displayName": "Essence Ring",
  "shortName": "essence_ring",
  "language": {
      "displayName": "Essence Ring",
      "lore": [],
      "description": [
          "<h1>Active: Life Essence</h1> Heal 425 and gain 425 max health for 15 seconds."
      ],
      "notes": [],
      "attributes": [
          "+2.5 Mana Regeneration",
          "+6 Intelligence"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          20.0
      ],
      "manaCost": [
          200.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "essence_ring_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"360": {
  "id": 360,
  "name": "item_clumsy_net",
  "displayName": "Clumsy Net",
  "shortName": "clumsy_net",
  "language": {
      "displayName": "Clumsy Net",
      "lore": [],
      "description": [
          "<h1>Active: Ensnare</h1> Ensnares the target enemy and yourself for 1.75 seconds.<br><br> Range: %abilitycastrange%"
      ],
      "notes": [],
      "attributes": [
          "+6 All Attributes",
          "+2 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 8,
      "unitTargetType": 3,
      "unitTargetTeam": 1,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          900
      ],
      "cooldown": [
          25.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "clumsy_net_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"361": {
  "id": 361,
  "name": "item_enchanted_quiver",
  "displayName": "Enchanted Quiver",
  "shortName": "enchanted_quiver",
  "language": {
      "displayName": "Enchanted Quiver",
      "lore": [],
      "description": [
          "<h1>Passive: Certain Strike</h1> Empowers your next attack with 270 bonus magical damage and True Strike. <font color='#7d7d7d'>Ranged Attackers</font> have +400 bonus range for the attack."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          8.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "enchanted_quiver_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"362": {
  "id": 362,
  "name": "item_ninja_gear",
  "displayName": "Ninja Gear",
  "shortName": "ninja_gear",
  "language": {
      "displayName": "Ninja Gear",
      "lore": [],
      "description": [
          "<h1>Active: Solitary Disguise</h1> Casts Smoke of Deceit on yourself only."
      ],
      "notes": [],
      "attributes": [
          "+20 Agility",
          "+30 Movement Speed"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          45.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ninja_gear_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"363": {
  "id": 363,
  "name": "item_illusionsts_cape",
  "displayName": "Illusionst's Cape",
  "shortName": "illusionsts_cape",
  "language": {
      "displayName": "Illusionist's Cape",
      "lore": [],
      "description": [
          "<h1>Active: Create Illusion</h1> Creates an image under your control. ",
          " <h1>Passive: Illusion Mastery</h1>Increases outgoing damage of all units and illusions controlled by the hero by 10%."
      ],
      "notes": [],
      "attributes": [
          "0% IMAGE DURATION:",
          "0% IMAGE DAMAGE:",
          "50% IMAGE DAMAGE TAKEN:",
          "+14 Agility",
          "+14 Strength"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          30.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "illusionsts_cape_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"364": {
  "id": 364,
  "name": "item_havoc_hammer",
  "displayName": "Havoc Hammer",
  "shortName": "havoc_hammer",
  "language": {
      "displayName": "Havoc Hammer",
      "lore": [],
      "description": [
          "<h1>Active: Havoc</h1> Knocks back enemies in 300 range around you, slowing them by 50 for 3 seconds and dealing 175 + 1x your str as magical damage."
      ],
      "notes": [],
      "attributes": [
          "+30 Damage",
          "+18 Strength"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          10.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "havoc_hammer_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"365": {
  "id": 365,
  "name": "item_panic_button",
  "displayName": "Magic Lamp",
  "shortName": "panic_button",
  "language": {
      "displayName": "Magic Lamp",
      "lore": [],
      "description": [
          "<h1>Passive: Rejuvenate </h1> When the wearer's health falls below 15, they will receive a hard dispel and be healed for 300 health."
      ],
      "notes": [],
      "attributes": [
          "+400 Health"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          75.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "panic_button_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"366": {
  "id": 366,
  "name": "item_apex",
  "displayName": "Apex",
  "shortName": "apex",
  "language": {
      "displayName": "Apex",
      "lore": [],
      "description": [
          "Increases your primary attribute by 80%."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "apex_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"367": {
  "id": 367,
  "name": "item_ballista",
  "displayName": "Ballista",
  "shortName": "ballista",
  "language": {
      "displayName": "Ballista",
      "lore": [],
      "description": [
          "<h1>Passive: Knockback</h1> Knocks back enemies with every attack."
      ],
      "notes": [],
      "attributes": [
          "+400 Attack Range <span className={s.abilityitemballista}>(Ranged Only)</span>",
          "0% KNOCKBACK DISTANCE:"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ballista_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"368": {
  "id": 368,
  "name": "item_woodland_striders",
  "displayName": "Woodland Striders",
  "shortName": "woodland_striders",
  "language": {
      "displayName": "Woodland Striders",
      "lore": [],
      "description": [
          "",
          "<h1>Active: Woodland Stride</h1> Create a path of trees behind you for 3 seconds. Trees last up to 60 seconds. ",
          "<h1>Passive: Tree Walking</h1> Removes movement speed limit and allows free pathing through trees.",
          " ",
          "Movement speed bonuses from multiple pairs of boots do not stack."
      ],
      "notes": [],
      "attributes": [
          "+50% Movement Speed",
          "+70 HP Regeneration"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          20.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "woodland_striders_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"370": {
  "id": 370,
  "name": "item_demonicon",
  "displayName": "Book of the Dead",
  "shortName": "demonicon",
  "language": {
      "displayName": "Book of the Dead",
      "lore": [],
      "description": [
          "<h1>Active: Greater Demonic Summoning</h1> Summon 6 Level 3 Necronomicon Units that last 75 seconds."
      ],
      "notes": [],
      "attributes": [
          "+35 Intelligence",
          "+35 Strength"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          90.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "demonicon_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"371": {
  "id": 371,
  "name": "item_fallen_sky",
  "displayName": "Fallen Sky",
  "shortName": "fallen_sky",
  "language": {
      "displayName": "Fallen Sky",
      "lore": [],
      "description": [
          "<h1>Active: Fallen Sky</h1> Transform into a meteor that strikes down at the target area after .5 seconds in a 315 AoE, stunning enemies for 2.0 seconds and dealing impact damage. Continues to deal damage every 1.0 seconds to enemy units and buildings for 6 seconds.<BR><BR>Building Impact Damage: 75 <BR>Building Over Time Damage: 50 <BR><BR>Non-Building Impact Damage: 150 <BR>Non-Building Over Time Damage: 90<BR><BR> Range: %abilitycastrange%"
      ],
      "notes": [
          "Does not get disabled by incoming damage."
      ],
      "attributes": [
          "+20 Strength",
          "+20 Intelligence",
          "+15.0 HP Regeneration",
          "+10.0 Mana Regeneration"
      ]
  },
  "stat": {
      "behavior": 5136,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          1600
      ],
      "cooldown": [
          15.0
      ],
      "sharedCooldown": "",
      "cost": 4751,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": true
  },
  "components": [
      {
          "index": 1,
          "componentId": 1
      },
      {
          "index": 2,
          "componentId": 223
      }
  ],
  "image": "fallen_sky_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": true
},
"372": {
  "id": 372,
  "name": "item_pirate_hat",
  "displayName": "Pirate Hat",
  "shortName": "pirate_hat",
  "language": {
      "displayName": "Pirate Hat",
      "lore": [],
      "description": [
          "<h1>Passive: Kill Bounty</h1> Creates a bounty rune between you and your victim when you kill an enemy hero."
      ],
      "notes": [],
      "attributes": [
          "+250 Attack Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "pirate_hat_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"373": {
  "id": 373,
  "name": "item_dimensional_doorway",
  "displayName": "Dimensional Doorway",
  "shortName": "dimensional_doorway",
  "language": {
      "displayName": "Dimensional Doorway",
      "lore": [],
      "description": [
          "<h1>Active</h1> Teleport instantly anywhere on the map."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 5136,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          90.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 3,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "dimensional_doorway_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"374": {
  "id": 374,
  "name": "item_ex_machina",
  "displayName": "Ex Machina",
  "shortName": "ex_machina",
  "language": {
      "displayName": "Ex Machina",
      "lore": [],
      "description": [
          "<h1>Active: Reset Item Cooldowns</h1> Reset the cooldown on all items (except Refresher Orb)."
      ],
      "notes": [],
      "attributes": [
          "+25 Armor"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          30.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "ex_machina_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"375": {
  "id": 375,
  "name": "item_faded_broach",
  "displayName": "Faded Broach",
  "shortName": "faded_broach",
  "language": {
      "displayName": "Faded Broach",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": [
          "+225 Mana",
          "+25 Movement Speed"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "faded_broach_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"376": {
  "id": 376,
  "name": "item_paladin_sword",
  "displayName": "Paladin Sword",
  "shortName": "paladin_sword",
  "language": {
      "displayName": "Paladin Sword",
      "lore": [],
      "description": [
          "<h1>Passive: Greater Healing</h1> Amplifies the wearer's Regeneration, Healing and Lifesteal by 17%."
      ],
      "notes": [],
      "attributes": [
          "+17 Damage",
          "+17% Lifesteal"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "paladin_sword_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"377": {
  "id": 377,
  "name": "item_minotaur_horn",
  "displayName": "Minotaur Horn",
  "shortName": "minotaur_horn",
  "language": {
      "displayName": "Minotaur Horn",
      "lore": [],
      "description": [
          "<h1>Active: Lesser Avatar</h1> Grants Spell Immunity for 1.75 seconds."
      ],
      "notes": [],
      "attributes": [
          "+20 Strength"
      ]
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "cooldown": [
          40.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "minotaur_horn_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"378": {
  "id": 378,
  "name": "item_orb_of_destruction",
  "displayName": "Orb of Destruction",
  "shortName": "orb_of_destruction",
  "language": {
      "displayName": "Orb of Destruction",
      "lore": [],
      "description": [
          "<h1>Passive: Impeding Corruption</h1> Attacks reduce the target's armor and movement speed. Slow amount varies based on whether the wearer is melee or ranged."
      ],
      "notes": [],
      "attributes": [
          "% ARMOR REDUCTION:",
          "0% SLOW (MELEE):",
          "5% SLOW (RANGE):",
          "% DURATION:"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "orb_of_destruction_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"379": {
  "id": 379,
  "name": "item_the_leveller",
  "displayName": "The Leveller",
  "shortName": "the_leveller",
  "language": {
      "displayName": "The Leveller",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": [
          "+50 Attack Speed",
          "5% BONUS BUILDING DAMAGE:"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "the_leveller_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"381": {
  "id": 381,
  "name": "item_titan_sliver",
  "displayName": "Titan Sliver",
  "shortName": "titan_sliver",
  "language": {
      "displayName": "Titan Sliver",
      "lore": [],
      "description": [
          "Increases the wearer's attack damage, magic and status resistance."
      ],
      "notes": [],
      "attributes": [
          "+16% Base Attack Damage",
          "+16% Magic Resistance",
          "+16% Status Resistance"
      ]
  },
  "stat": {
      "behavior": 2,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "titan_sliver_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1021": {
  "id": 1021,
  "name": "item_river_painter",
  "displayName": "River Vial: Chrome",
  "shortName": "river_painter",
  "language": {
      "displayName": "River Vial: Chrome",
      "lore": [],
      "description": [
          "Pour this serum into the river to transform the water into liquid chrome for 900.0 seconds. <font color='#FF5555'>Using this item on the river will permanently consume one charge from your Armory supply upon expiration of the effect. Charges do not get consumed if the effect is replaced by a stronger vial before expiration.</font> Vials can only be used if at least one charge remains in your Armory. Charges will only be used by successfully enchanting the river, and will not be consumed if the item is sold, dropped, or destroyed during a game. You can only cast a vial on the river if there's not a stronger vial already in effect."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          200
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "aliases": "paint",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 0,
      "initialStock": 1,
      "stockTime": 900,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "river_painter_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1022": {
  "id": 1022,
  "name": "item_river_painter2",
  "displayName": "River Vial: Dry",
  "shortName": "river_painter2",
  "language": {
      "displayName": "Recipe: ",
      "lore": [],
      "description": [
          "Pour this serum into the river to dry up the water for 900.0 seconds. <font color='#FF5555'>Using this item on the river will permanently consume one charge from your Armory supply upon expiration of the effect . Charges do not get consumed if the effect is replaced by a stronger vial before expiration.</font> Vials can only be used if at least one charge remains in your Armory. Charges will only be used by successfully enchanting the river, and will not be consumed if the item is sold, dropped, or destroyed during a game. You can only cast a vial on the river if there's not a stronger vial already in effect."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          200
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "aliases": "paint",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 0,
      "initialStock": 1,
      "stockTime": 900,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "river_painter2_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1023": {
  "id": 1023,
  "name": "item_river_painter3",
  "displayName": "River Vial: Slime",
  "shortName": "river_painter3",
  "language": {
      "displayName": "Recipe: ",
      "lore": [],
      "description": [
          "Pour this serum into the river to transform the water into a green slime for 900.0 seconds. <font color='#FF5555'>Using this item on the river will permanently consume one charge from your Armory supply upon expiration of the effect. Charges do not get consumed if the effect is replaced by a stronger vial before expiration.</font> Vials can only be used if at least one charge remains in your Armory. Charges will only be used by successfully enchanting the river, and will not be consumed if the item is sold, dropped, or destroyed during a game. You can only cast a vial on the river if there's not a stronger vial already in effect."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          200
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "aliases": "paint",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 0,
      "initialStock": 1,
      "stockTime": 900,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "river_painter3_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1024": {
  "id": 1024,
  "name": "item_river_painter4",
  "displayName": "River Vial: Oil",
  "shortName": "river_painter4",
  "language": {
      "displayName": "Recipe: ",
      "lore": [],
      "description": [
          "Pour this serum into the river to make the water oily for 900.0 seconds. <font color='#FF5555'>Using this item on the river will permanently consume one charge from your Armory supply upon expiration of the effect. Charges do not get consumed if the effect is replaced by a stronger vial before expiration.</font> Vials can only be used if at least one charge remains in your Armory. Charges will only be used by successfully enchanting the river, and will not be consumed if the item is sold, dropped, or destroyed during a game. You can only cast a vial on the river if there's not a stronger vial already in effect."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          200
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "aliases": "paint",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 0,
      "initialStock": 1,
      "stockTime": 900,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "river_painter4_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1025": {
  "id": 1025,
  "name": "item_river_painter5",
  "displayName": "River Vial: Electrified",
  "shortName": "river_painter5",
  "language": {
      "displayName": "Recipe: ",
      "lore": [],
      "description": [
          "Pour this serum into the river to make the water electric for 900.0 seconds. <font color='#FF5555'>Using this item on the river will permanently consume one charge from your Armory supply upon expiration of the effect. Charges do not get consumed if the effect is replaced by a stronger vial before expiration.</font> Vials can only be used if at least one charge remains in your Armory. Charges will only be used by successfully enchanting the river, and will not be consumed if the item is sold, dropped, or destroyed during a game. You can only cast a vial on the river if there's not a stronger vial already in effect."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          200
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "aliases": "paint",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 0,
      "initialStock": 1,
      "stockTime": 900,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "river_painter5_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1026": {
  "id": 1026,
  "name": "item_river_painter6",
  "displayName": "River Vial: Potion",
  "shortName": "river_painter6",
  "language": {
      "displayName": "Recipe: ",
      "lore": [],
      "description": [
          "Pour this serum into the river to transform the water into a purple bubbling potion for 900.0 seconds. <font color='#FF5555'>Using this item on the river will permanently consume one charge from your Armory supply upon expiration of the effect. Charges do not get consumed if the effect is replaced by a stronger vial before expiration.</font> Vials can only be used if at least one charge remains in your Armory. Charges will only be used by successfully enchanting the river, and will not be consumed if the item is sold, dropped, or destroyed during a game. You can only cast a vial on the river if there's not a stronger vial already in effect."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          200
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "aliases": "paint",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 0,
      "initialStock": 1,
      "stockTime": 900,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "river_painter6_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1027": {
  "id": 1027,
  "name": "item_river_painter7",
  "displayName": "River Vial: Blood",
  "shortName": "river_painter7",
  "language": {
      "displayName": "Recipe: ",
      "lore": [],
      "description": [
          "Pour this serum into the river to transform the water into blood for 900.0 seconds. <font color='#FF5555'>Using this item on the river will permanently consume one charge from your Armory supply upon expiration of the effect. Charges do not get consumed if the effect is replaced by a stronger vial before expiration.</font> Vials can only be used if at least one charge remains in your Armory. Charges will only be used by successfully enchanting the river, and will not be consumed if the item is sold, dropped, or destroyed during a game. You can only cast a vial on the river if there's not a stronger vial already in effect."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          200
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "aliases": "paint",
      "quality": "component",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": true,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 1,
      "initialCharges": 0,
      "initialStock": 1,
      "stockTime": 900,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "river_painter7_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1028": {
  "id": 1028,
  "name": "item_mutation_tombstone",
  "displayName": "Tombstone",
  "shortName": "mutation_tombstone",
  "language": {
      "displayName": "Tombstone",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 32900,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          100
      ],
      "castPoint": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "shopTags": "consumable",
      "quality": "consumable",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": true,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 1,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "mutation_tombstone_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1029": {
  "id": 1029,
  "name": "item_super_blink",
  "displayName": "Super Blink Dagger",
  "shortName": "super_blink",
  "language": {
      "displayName": "Super Blink Dagger",
      "lore": [
          "The fabled dagger used by the fastest assassin ever to walk the lands."
      ],
      "description": [
          "<h1>Active: Blink</h1> Teleport to a target point up to 1200 units away."
      ],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 5136,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          0
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          15.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "super_blink_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1030": {
  "id": 1030,
  "name": "item_pocket_tower",
  "displayName": "Pocket Tower",
  "shortName": "pocket_tower",
  "language": {
      "displayName": "Pocket Tower",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 16,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castRange": [
          600
      ],
      "castPoint": [
          0.0
      ],
      "cooldown": [
          15.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 0,
      "quality": "false",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "pocket_tower_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
},
"1032": {
  "id": 1032,
  "name": "item_pocket_roshan",
  "displayName": "Pocket Roshan",
  "shortName": "pocket_roshan",
  "language": {
      "displayName": "Pocket Roshan",
      "lore": [],
      "description": [],
      "notes": [],
      "attributes": []
  },
  "stat": {
      "behavior": 2052,
      "unitTargetType": 0,
      "unitTargetTeam": 0,
      "unitTargetFlags": 0,
      "fightLevelRecap": 0,
      "castPoint": [
          0.0
      ],
      "cooldown": [
          60.0
      ],
      "manaCost": [
          0.0
      ],
      "sharedCooldown": "",
      "cost": 1000,
      "quality": "rare",
      "isSellable": true,
      "isDroppable": true,
      "isPurchaseable": false,
      "isSecretShop": false,
      "isSideShop": false,
      "isStackable": false,
      "isPermanent": false,
      "isHideCharges": false,
      "isRequiresCharges": false,
      "isDisplayCharges": false,
      "isSupport": false,
      "isTempestDoubleClonable": false,
      "stockMax": 0,
      "initialCharges": 0,
      "initialStock": 0,
      "stockTime": 0,
      "initialStockTime": 0,
      "isRecipe": false,
      "needsComponents": false
  },
  "image": "pocket_roshan_lg.png?3",
  "isInNeuralNetwork": false,
  "isFullItemHeroPurchaseItem": false
}